...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Action_baseURL"] = "/transaction/adjustment_entry/details?rights=" + rights
	tconf["ActionCol_param"] = "ID"
	//tconf["hdr_id"] = ""
	tconf["Interpret_button"] = "false"
	tconf["Add_post_button"] = "false"
	tconf["Add_post_button_url"] = "/transaction/adjustment_entry/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["ActionCol_void"] = "false" // config for void click

	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/transaction/adjustment_entry/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["withtotal"] = "true"
	tconf["Archive"] = "false"
	tconf["org"] = str_OrgID
	tconf["Add_duplicate_button"] = "false"
	tconf["Add_duplicate_button_url"] = "/transaction/adjustment_entry/details/duplicate?rights=" + rights + "&id=" + tconf["hdr_id"]
	...
	if ln == "" {
		...
		arr_data := datatables.Data_row(`exec AEHDR_Get 1,` + tconf["hdr_id"])
		fmt.Println(arr_data)
		tconf["id"] = arr_data[0]
		tconf["trandate"] = arr_data[1]
		tconf["aetype"] = arr_data[2]
		...
		tconf["interest"] = arr_data[8]
		tconf["last_update"] = arr_data[9]
		tconf["textfiledownloaded"] = arr_data[10]
		tconf["textfiledownloaded_by"] = arr_data[11]
		tconf["searchtext"] = arr_data[12]
		tconf["org"] = arr_data[13]
		tconf["added"] = arr_data[14]
		tconf["added_by"] = arr_data[15]
		tconf["updated"] = arr_data[16]
		tconf["updated_by"] = arr_data[17]
		tconf["status"] = arr_data[18]
		tconf["AEType_name"] = arr_data[19]
		tconf["dtl_amt"] = arr_data[20]

		tran_status := arr_data[18]
		tconf["Add_new_button_url"] = "/transaction/adjustment_entry/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"] + "&member_name=" + tconf["member_name"]
		...
		var ajaxURLdata = "/transaction/adjustment_entry/details?rights=" + rights + "&ID=" + tconf["hdr_id"]
		...
		if tmpl, err = tmpl.ParseFiles("transaction/adjustment_entry/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol_edit_url"] = "/transaction/adjustment_entry/details/DEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "false"                                                                            // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/adjustment_entry/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `AEDTL_List 1, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `AEDTL_List 0, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		*/
		...
		var err error
		tmpl := template.New("Dadd.html")
		if tmpl, err = tmpl.ParseFiles("transaction/adjustment_entry/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		...
		fmt.Println(`AEDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + r.Form["member"][0] + `,` + r.Form["trantype"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		arr_raw_data := datatables.Data_row(`AEDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + r.Form["member"][0] + `,` + r.Form["trantype"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec AEDTL_Get 1, ` + tconf["h_id"] + `,0`)
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/adjustment_entry/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_trantype})
		if err1 != nil {
		...
		arr_raw_data := datatables.Data_row(`AEDTL_SaveEdit '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,` + r.Form["member"][0] + `,` + r.Form["trantype"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/adjustment_entry/details/DDeleteHandler"
		tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/adjustment_entry/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`AEDTL_SaveDelete '` + username + `',` + r.Form["h_id"][0])
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`AEHDR_Post '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`AEHDR_Cancel '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func Duplicate(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`AEHDR_Duplicate  '` + username + `' ,  ` + tconf["id"])
	arr_data := datatables.Data_row(`AEHDR_Duplicate  '` + username + `' ,  ` + tconf["id"])
	js, err := json.Marshal(arr_data)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
