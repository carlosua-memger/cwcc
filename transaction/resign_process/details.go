...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Resign Process Detail"
	...
	tconf["Add_post_button_url"] = "/transaction/resign_process/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["Add_download_button_url"] = "/transaction/resign_process/details/download?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["ActionCol_void"] = "false" // config for void click
	tconf["Add_clearance_button"] = "true"

	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/transaction/resign_process/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	...
	var header = []string{"Action", "Member", "Transaction Type", "Loan Amortization", "Amount", "Remarks"} //column header
	if ln == "" {

		tconf["ismoreinfo"] = "true" // to description info
		arr_data := datatables.Data_row(`exec ARLR_Get 1,` + tconf["hdr_id"])
		...
		tran_status := arr_data[20]
		tconf["Add_new_button_url"] = "/transaction/resign_process/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"] + "&member_name=" + tconf["member_name"]
		tconf["Download_LoanBal_url"] = "/transaction/resign_process/details/DownloadLoanBal?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"]
		...
		var ajaxURLdata = "/transaction/resign_process/details?rights=" + rights + "&ID=" + tconf["hdr_id"]
		...
		tconf["ActionCol_edit_url"] = "/transaction/resign_process/details/DEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "false"                                                                          // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/resign_process/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `ARLRDtl_List 1, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `ARLRDtl_List 0, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		*/
		...
		var err error
		tmpl := template.New("Dadd.html")
		if tmpl, err = tmpl.ParseFiles("transaction/resign_process/Dadd.html"); err != nil {
			fmt.Println(err)
		}
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/resign_process/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/resign_process/details/DDeleteHandler"
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`ARLR_Post '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DDownloadHandler(w http.ResponseWriter, r *http.Request) {
	hdr_id := r.URL.Query().Get("hdr_id")
	username, _ := login.Get_account_info(r)
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`ARLR_Upload '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`ARLR_Cancel '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
