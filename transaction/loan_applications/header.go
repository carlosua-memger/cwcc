...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Loan Applications"
	tconf["Add_new_button"] = "false"
	tconf["Rights"] = rights
	tconf["Add_back_button"] = "false"
	tconf["Add_new_button_details"] = "false"
	tconf["upload_button"] = "false"
	tconf["Add_new_button_url"] = "/transaction/loan_applications/HaddHandler?rights=" + rights
	tconf["Action_baseURL"] = "/transaction/loan_applications/details?rights=" + rights
	...
	var header = []string{"Action", "ID", "Status", "Received", "Trandate", "Member", "Partner", "Loantype", "Purpose", "Amount", "Amount Generated", "Term", "Payment Mode", "Coordinator", "Endorsed", "Remarks"} //column header
	if ln == "" {
		// content access here
		...
		var ajaxURLdata = "/transaction/loan_applications?rights=" + rights
		type TblConf struct {
			Headercol []string
			AjaxUrl   string
		...
		if tmpl, err = tmpl.ParseFiles("transaction/loan_applications/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol_edit_url"] = "/transaction/loan_applications/HEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_view"] = "false"                                                                       // config for edit click
		tconf["ActionCol_view_is_modal"] = "false"                                                              // config for edit click
		//tconf["ActionCol_edit_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click
		tconf["ActionCol_view_url"] = "/transaction/loan_applications/HViewHandler?rights=" + rights + "&h_id=" // config for edit click

		tconf["ActionCol_delete"] = "false" // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/loan_applications/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `la_List 1, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `la_List 0, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_applications/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_partner, arr_data_loantype, arr_data_paymodes, arr_data_coor})
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		tconf["LoanConso_url"] = "/transaction/loan_consolidation/details?rights=" + rights + "&hdr_id=" + tconf["h_id"] + "&member=" + tconf["la_member"] + "&isposted=" + tconf["isposted"]
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_applications/Hedit.html"); err != nil {
			fmt.Println(err)
		}
...
func HViewHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		//tconf["Add_upload_button_url"] = "/transaction/loan_release/HaddHandler?rights=" + rights + "&fromtype=la&la_id=" + tconf["h_id"]
		tconf["CreateLR_url"] = "/transaction/loan_release/HaddHandler?rights=" + rights + "&fromtype=la&la_id=" + tconf["h_id"]
		tconf["LoanConso_url"] = "/transaction/loan_consolidation/details?rights=" + rights + "&hdr_id=" + tconf["h_id"] + "&member=" + tconf["la_member"] + "&isposted=1&fromlr=2"
		//arr_data_itemclass := datatables.DataList(`select id,dbo.SIS_Itemclass_Name(1, 3, id) [itemclass_name] from sis_itemclass`  )
		//arr_data_supplier := datatables.DataList(`select id,dbo.SIS_Supplier_Name(1, 3, id) [supplier_name] from SIS_Supplier`  )
		//arr_data_itemclass := datatables.DataList(`SIS_ItemClass_Get 2,`+str_OrgID  )
		//arr_data_supplier := datatables.DataList(`SIS_Supplier_Get 2,`+str_OrgID  )
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/loan_applications/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`LA_SaveDelete '` + username + `' , ` + str_OrgID + ` , ` + r.Form["h_id"][0] + ``)
		/*
			fmt.Println(arr_data)
