...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Loan Payment Detail"
	...
	tconf["Add_post_button_url"] = "/transaction/loan_payment/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["ActionCol_void"] = "false" // config for void click
	tconf["Add_exception_button"] = "true"

	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/transaction/loan_payment/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["withtotal"] = "true"
	tconf["Archive"] = "false"
	tconf["org"] = str_OrgID
	tconf["Add_duplicate_button"] = "false"
	tconf["Add_duplicate_button_url"] = "/transaction/loan_payment/details/duplicate?rights=" + rights + "&id=" + tconf["hdr_id"]
	...
	var header = []string{"Action", "Member", "Loan Type", "Loan Amortization", "Amount", "Remarks"} //column header
	if ln == "" {

		tconf["ismoreinfo"] = "true" // to description info
		arr_data := datatables.Data_row(`exec LPHDR_Get 2,` + tconf["hdr_id"])
		...
		tconf["dtl_total"] = arr_data[25]

		tran_status := arr_data[21]
		tconf["Add_new_button_url"] = "/transaction/loan_payment/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"] + "&member_name=" + tconf["member_name"]
		tconf["Download_LoanBal_url"] = "/transaction/loan_payment/details/DownloadLoanBal?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"]
		tconf["Add_deleteall_button_url"] = "/transaction/loan_payment/details/deleteall?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
		...
		var ajaxURLdata = "/transaction/loan_payment/details?rights=" + rights + "&ID=" + tconf["hdr_id"]
		...
		tconf["ActionCol_edit_url"] = "/transaction/loan_payment/details/DEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "false"                                                                        // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/loan_payment/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `LPDTL_List 1, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `LPDTL_List 0, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arrDataLoans := datatables.DataList(`exec LRDTL_Get 4,` + member + `,` + hdr_id)
		...
		*/
		...
		var err error
		tmpl := template.New("Dadd.html")
		if tmpl, err = tmpl.ParseFiles("transaction/loan_payment/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		member := r.Form["member_id"][0]
		if member == "NULL" {
			member = "0"
			member = r.Form["member"][0]
		}
		fmt.Println(`LPDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + member + `,` + r.Form["lrdtl"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		arr_raw_data := datatables.Data_row(`LPDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + member + `,` + r.Form["lrdtl"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		fmt.Println(`exec LPDTL_Get 3, 0,` + tconf["h_id"])
		arr_data := datatables.Data_row(`exec LPDTL_Get 1, ` + tconf["h_id"] + `,0`)
		...
		tconf["LoanTypeName"] = arr_data[13]
		tconf["LRDTLName"] = arr_data[14]

		tmpl := template.New("Dedit.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_payment/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
		...
		fmt.Println(r.Form["hdr_id"][0])
		fmt.Println(r.Form["member_id"][0])
		fmt.Println(r.Form["lrdtl"][0])
		fmt.Println(r.Form["amount"][0])
		fmt.Println(r.Form["remarks"][0])
		arr_raw_data := datatables.Data_row(`LPDTL_SaveEdit '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,` + r.Form["member_id"][0] + `,` + r.Form["lrdtl"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/loan_payment/details/DDeleteHandler"
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`LPDTL_SaveDelete '` + username + `',` + r.Form["h_id"][0])
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`LPHDR_Post '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`LPHDR_Cancel '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func Download_LoanBalHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		html_file = "transaction/loan_payment/download_LoanBal.html"
...
func UploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		dst, err := os.Create("textfiles/upload_lp/" + datetimefoldername + "_" + partner + "_" + part.FileName())
		defer dst.Close()
		...
		contents, err := ioutil.ReadFile("textfiles/upload_lp/" + datetimefoldername + "_" + partner + "_" + part.FileName())
		if err != nil {
			...
			if key > 0 && key < (len(lines)) {
				...
				if len(string(lines[key])) > 29 {
					...
					if strings.Contains(string(lines[key]), "^") == false && amount_check > 0 {
						lrid := strings.TrimLeft(string(lines[key][0:8]), "0")
...
func DDeleteAllHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`LPHDR_SavePurgeDetail '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func Duplicate(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`LPHDR_Duplicate  '` + username + `' ,  ` + tconf["id"])
	arr_data := datatables.Data_row(`LPHDR_Duplicate  '` + username + `' ,  ` + tconf["id"])
	js, err := json.Marshal(arr_data)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
