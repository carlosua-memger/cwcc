...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Merge Member"
	tconf["Add_new_button"] = "false"
	tconf["Rights"] = rights
	tconf["Add_back_button"] = "false"
	tconf["Add_new_button_details"] = "false"
	tconf["upload_button"] = "false"
	tconf["Add_new_button_url"] = "/transaction/merge_member/HaddHandler?rights=" + rights
	tconf["Action_baseURL"] = "/transaction/merge_member/details?rights=" + rights
	...
	var header = []string{"Action", "ID", "Status", "Trandate", "Purge Member Name", "Retain Member Name", "Remarks"} //column header
	if ln == "" {
		// content access here
		...
		var ajaxURLdata = "/transaction/merge_member?rights=" + rights
		type TblConf struct {
			Headercol []string
			AjaxUrl   string
		...
		if tmpl, err = tmpl.ParseFiles("transaction/merge_member/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol_edit_url"] = "/transaction/merge_member/HEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_view"] = "false"                                                                  // config for edit click
		tconf["ActionCol_view_is_modal"] = "false"                                                         // config for edit click
		//tconf["ActionCol_edit_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click
		tconf["ActionCol_view_url"] = "/transaction/merge_member/HViewHandler?rights=" + rights + "&h_id=" // config for edit click

		tconf["ActionCol_delete"] = "false" // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/merge_member/HDeleteHandler?h_id="
		tconf["ActionCol_detail"] = "false"    // config for details click
		tconf["ActionCol_add_child"] = "false" // config for addchild click
		tconf["ActionCol_loanconsolidation_url"] = "/transaction/merge_member/details/DListHandler?h_id="
		tconf["style_size"] = "12"
		tconf["style_font"] = ""
		...
		tconf["sql_total"] = `Merge_List 1, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `Merge_List 0, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/merge_member/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec Merge_Get 1,` + tconf["h_id"])

		fmt.Println(arr_data)
		tconf["id"] = arr_data[0]
		tconf["trandate"] = arr_data[1]
		tconf["RID"] = arr_data[2]
		tconf["PID"] = arr_data[3]
		tconf["PName"] = arr_data[4]
		tconf["remarks"] = arr_data[5]
		tconf["lastTouched"] = arr_data[6]
		tconf["lastTouchedBy"] = arr_data[7]
		tconf["posted"] = arr_data[8]
		tconf["postedBy"] = arr_data[9]
		tconf["searchText"] = arr_data[10]
		tconf["org"] = arr_data[11]
		tconf["Status"] = arr_data[12]
		tconf["Retain_Member_Name"] = arr_data[13]
		tconf["Purge_Member_Name"] = arr_data[14]
		tconf["Add_post_button_url"] = "/transaction/merge_member/post?rights=" + rights + "&hdr_id=" + tconf["h_id"]
		tconf["Action_button_save"] = "true"

		if arr_data[12] == `For Posting` {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/merge_member/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/merge_member/HDeleteHandler"
		tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/merge_member/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		arr_data := datatables.Data_row(`Merge_SaveDelete '` + username + `' , ` + r.Form["h_id"][0] + ``)
		/*
			fmt.Println(arr_data)
...
func HPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`Merge_Post '` + username + `',` + hdr_id)
