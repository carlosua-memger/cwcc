...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Loan Release Detail"
	tconf["Add_new_button"] = "false"
	tconf["Rights"] = rights
	tconf["Add_back_button"] = "true"
	tconf["Add_new_button_details"] = "false"
	tconf["upload_button"] = "false"
	tconf["Add_new_button_url"] = "/transaction/loan_release/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	...
	tconf["Add_post_button_url"] = "/transaction/loan_release/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["ActionCol_void"] = "false" // config for void click
	tconf["ActionCol_void_url"] = "/transaction/loan_release/details/DVoidHandler?h_id="
	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/transaction/loan_release/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["withtotal"] = "true"
	tconf["Archive"] = "false"
	tconf["org"] = str_OrgID
	tconf["Add_newla_button_url"] = "/transaction/loan_applications/HaddHandler?rights=" + rights
	...
	var header = []string{"Action", "Due", "Due Amt", "Principal", "Interest", "Service Fee", "Remarks"} //column header
	if ln == "" {
		...
		tconf["LoanConso_url"] = "/transaction/loan_consolidation/details?rights=" + rights + "&hdr_id=" + tconf["la"] + "&member=" + tconf["member"] + "&fromlr=1&lrid=" + tconf["id"] + "&isposted=" + tconf["isposted"]

		var ajaxURLdata = "/transaction/loan_release/details?rights=" + rights + "&ID=" + tconf["hdr_id"]
		...
		tconf["ActionCol_edit_url"] = "/transaction/loan_release/details/DEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "false"                                                                        // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/loan_release/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `LRDTL_List 1, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `LRDTL_List 0, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		*/
		...
		var err error
		tmpl := template.New("Dadd.html")
		if tmpl, err = tmpl.ParseFiles("transaction/loan_release/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
		...
		r.ParseForm()
		fmt.Println(`LRDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + r.Form["due"][0] + `,` + r.Form["principal"][0] + `,` + r.Form["interest"][0] + `,'` + r.Form["remarks"][0] + `,'` + r.Form["service_fee"][0])
		arr_raw_data := datatables.Data_row(`LRDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,'` + r.Form["due"][0] + `',` + r.Form["principal"][0] + `,` + r.Form["interest"][0] + `,'` + r.Form["remarks"][0] + `',` + r.Form["service_fee"][0])
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		fmt.Println(`exec LRDTL_Get 3, 0,` + tconf["h_id"])
		arr_data := datatables.Data_row(`exec LRDTL_Get 3, ` + tconf["h_id"] + `,0`)

		fmt.Println(arr_data)
		tconf["id"] = arr_data[0]
		tconf["hdr"] = arr_data[1]
		tconf["due"] = arr_data[2]
		tconf["principal"] = arr_data[3]
		tconf["interest"] = arr_data[4]
		tconf["service_fee"] = arr_data[5]
		tconf["remarks"] = arr_data[6]

		tmpl := template.New("Dedit.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_release/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
		...
		fmt.Println(`LRDTL_SaveEdit '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,'` + r.Form["due"][0] + `',` + r.Form["principal"][0] + `,` + r.Form["interest"][0] + `,'` + r.Form["remarks"][0] + `,'` + r.Form["service_fee"][0])
		arr_raw_data := datatables.Data_row(`LRDTL_SaveEdit '` + username + `',` + r.Form["id"][0] + `, ` + r.Form["hdr_id"][0] + ` ,'` + r.Form["due"][0] + `',` + r.Form["principal"][0] + `,` + r.Form["interest"][0] + `,'` + r.Form["remarks"][0] + `',` + r.Form["service_fee"][0])
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/loan_release/details/DDeleteHandler"
		tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_release/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`LRDTL_SaveDelete '` + username + `',` + r.Form["h_id"][0])
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`LRHDR_Post '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`LRHDR_Cancel '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
