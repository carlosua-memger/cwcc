...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Acknowledgement Receipt"
	tconf["Add_new_button"] = "false"
	tconf["Rights"] = rights
	tconf["Add_back_button"] = "false"
	tconf["Add_new_button_details"] = "false"
	tconf["upload_button"] = "false"
	tconf["Add_new_button_url"] = "/transaction/acknowledgement_receipt/HaddHandler?rights=" + rights
	...
	var header = []string{"Action", "ID", "Status", "Trandate", "Partner", "Remarks"} //column header
	if ln == "" {
		// content access here
		...
		var ajaxURLdata = "/transaction/acknowledgement_receipt?rights=" + rights
		type TblConf struct {
			Headercol []string
			AjaxUrl   string
		...
		tconf["ActionCol_edit_url"] = "/transaction/acknowledgement_receipt/HEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_view"] = "false"                                                                             // config for edit click
		tconf["ActionCol_view_is_modal"] = "false"                                                                    // config for edit click
		//tconf["ActionCol_edit_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click
		tconf["ActionCol_view_url"] = "/transaction/acknowledgement_receipt/HViewHandler?rights=" + rights + "&h_id=" // config for edit click

		tconf["ActionCol_delete"] = "false" // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/acknowledgement_receipt/HDeleteHandler?h_id="
		tconf["ActionCol_detail"] = "true"     // config for details click
		tconf["ActionCol_add_child"] = "false" // config for addchild click
		tconf["ActionCol_loanconsolidation_url"] = "/transaction/acknowledgement_receipt/details/DListHandler?h_id="
		tconf["style_size"] = "12"
		tconf["style_font"] = ""
		...
		tconf["sql_total"] = `ARHDR_List 1, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `ARHDR_List 0, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/acknowledgement_receipt/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_partner})
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec ARHDR_Get 1,` + tconf["h_id"])
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/acknowledgement_receipt/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_partner})
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/acknowledgement_receipt/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`ARHDR_SaveDelete '` + username + `' , ` + r.Form["h_id"][0] + ``)
		/*
			fmt.Println(arr_data)
