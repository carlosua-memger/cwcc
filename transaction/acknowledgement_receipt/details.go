package acknowledgement_receipt
...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Acknowledgement Receipt Detail"
	...
	tconf["Action_baseURL"] = "/transaction/acknowledgement_receipt/details?rights=" + rights
	tconf["ActionCol_param"] = "ID"
	//tconf["hdr_id"] = ""
	tconf["Interpret_button"] = "false"
	tconf["Add_post_button"] = "false"
	tconf["Add_post_button_url"] = "/transaction/acknowledgement_receipt/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]

	tconf["Add_deleteall_button_url"] = "/transaction/acknowledgement_receipt/details/deleteall?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["ActionCol_void"] = "false" // config for void click
	tconf["Add_duplicate_button"] = "false"
	tconf["Add_duplicate_button_url"] = "/transaction/acknowledgement_receipt/details/duplicate?rights=" + rights + "&id=" + tconf["hdr_id"]

	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/transaction/acknowledgement_receipt/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]

	tconf["withtotal"] = "true"
	tconf["Archive"] = "false"
	tconf["org"] = str_OrgID
	tconf["Add_exception_button"] = "true"
	...
	var header = []string{"Action", "Member", "Transaction Type", "Amount", "Remarks"} //column header
	if ln == "" {

		tconf["ismoreinfo"] = "true" // to description info
		arr_data := datatables.Data_row(`exec ARHDR_Get 2,` + tconf["hdr_id"])
		...
		tconf["last_update"] = arr_data[8]
		tconf["searchtext"] = arr_data[9]
		tconf["org"] = arr_data[10]
		tconf["added"] = arr_data[11]
		tconf["added_by"] = arr_data[12]
		tconf["updated"] = arr_data[13]
		tconf["updated_by"] = arr_data[14]
		tconf["status"] = arr_data[15]
		tconf["PartnerName"] = arr_data[16]
		tconf["dtl_total"] = arr_data[17]

		tran_status := arr_data[15]
		tconf["Add_new_button_url"] = "/transaction/acknowledgement_receipt/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"] + "&member_name=" + tconf["member_name"]
		...
		var ajaxURLdata = "/transaction/acknowledgement_receipt/details?rights=" + rights + "&ID=" + tconf["hdr_id"]
		...
		if tmpl, err = tmpl.ParseFiles("transaction/acknowledgement_receipt/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol_edit_url"] = "/transaction/acknowledgement_receipt/details/DEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "false"                                                                                   // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/acknowledgement_receipt/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `ARDTL_List 1, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `ARDTL_List 0, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		*/
		...
		var err error
		tmpl := template.New("Dadd.html")
		if tmpl, err = tmpl.ParseFiles("transaction/acknowledgement_receipt/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		...
		fmt.Println(`ARDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + r.Form["member"][0] + `,` + r.Form["trantype"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		arr_raw_data := datatables.Data_row(`ARDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + r.Form["member"][0] + `,` + r.Form["trantype"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec ARDTL_Get 1, ` + tconf["h_id"] + `,0`)
		...
		tconf["amt"] = arr_data[4]
		tconf["remarks"] = arr_data[5]
		tconf["last_update"] = arr_data[6]
		tconf["added"] = arr_data[7]
		tconf["added_by"] = arr_data[8]
		tconf["updated"] = arr_data[9]
		tconf["updated_by"] = arr_data[10]
		tconf["MemberName"] = arr_data[11]
		tconf["TranTypeName"] = arr_data[12]

		arr_data_trantype := datatables.DataList(`exec TranType_Get 3, 0`)
		tmpl := template.New("Dedit.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/acknowledgement_receipt/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_trantype})
		if err1 != nil {
		...
		arr_raw_data := datatables.Data_row(`ARDTL_SaveEdit '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,` + r.Form["member"][0] + `,` + r.Form["trantype"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/acknowledgement_receipt/details/DDeleteHandler"
		tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/acknowledgement_receipt/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`ARDTL_SaveDelete '` + username + `',` + r.Form["h_id"][0])
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`ARHDR_Post '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`ARHDR_Cancel '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func Download_LoanBalHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		html_file = "transaction/acknowledgement_receipt/download_LoanBal.html"
...
func UploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	workCh := make(chan *ArDtlUploadReq, 50)
	initWorkers(workCh)
