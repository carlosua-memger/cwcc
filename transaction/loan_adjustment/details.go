...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Loan Adjustment Detail"
	...
	tconf["Action_baseURL"] = "/transaction/loan_adjustment/details?rights=" + rights
	tconf["ActionCol_param"] = "ID"
	//tconf["hdr_id"] = ""
	tconf["Interpret_button"] = "false"
	tconf["Add_post_button"] = "false"
	tconf["Add_post_button_url"] = "/transaction/loan_adjustment/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["ActionCol_void"] = "false" // config for void click

	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/transaction/loan_adjustment/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	tconf["withtotal"] = "true"
	tconf["Archive"] = "false"
	tconf["org"] = str_OrgID
	tconf["Add_duplicate_button"] = "false"
	tconf["Add_duplicate_button_url"] = "/transaction/loan_adjustment/details/duplicate?rights=" + rights + "&id=" + tconf["hdr_id"]
	...
	if ln == "" {
		...
		arr_data := datatables.Data_row(`exec LAEHDR_Get 1,` + tconf["hdr_id"])
		fmt.Println(arr_data)
		tconf["id"] = arr_data[0]
		tconf["trandate"] = arr_data[1]
		tconf["laetype"] = arr_data[2]
		...
		tconf["textfiledownloaded"] = arr_data[9]
		tconf["textfiledownloaded_by"] = arr_data[10]
		tconf["searchtext"] = arr_data[11]
		tconf["org"] = arr_data[12]
		tconf["added"] = arr_data[13]
		tconf["added_by"] = arr_data[14]
		tconf["updated"] = arr_data[15]
		tconf["updated_by"] = arr_data[16]
		tconf["status"] = arr_data[17]
		tconf["laetype_name"] = arr_data[18]
		tconf["dtl_amt"] = arr_data[19]

		tran_status := arr_data[17]
		tconf["Add_new_button_url"] = "/transaction/loan_adjustment/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&member=" + tconf["member"] + "&member_name=" + tconf["member_name"]
		...
		var ajaxURLdata = "/transaction/loan_adjustment/details?rights=" + rights + "&ID=" + tconf["hdr_id"]
		...
		if tmpl, err = tmpl.ParseFiles("transaction/loan_adjustment/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol_edit_url"] = "/transaction/loan_adjustment/details/DEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "false"                                                                           // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/loan_adjustment/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `LAEDTL_List 1, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `LAEDTL_List 0, ` + tconf["hdr_id"] + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		*/
		...
		var err error
		tmpl := template.New("Dadd.html")
		if tmpl, err = tmpl.ParseFiles("transaction/loan_adjustment/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
		...
		fmt.Println(`LAEDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + member + `,` + r.Form["lrdtl"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		arr_raw_data := datatables.Data_row(`LAEDTL_SaveAdd '` + username + `',` + r.Form["hdr_id"][0] + ` ,` + member + `,` + r.Form["lrdtl"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		fmt.Println(`exec LAEDTL_Get 3, 0,` + tconf["h_id"])
		arr_data := datatables.Data_row(`exec LAEDTL_Get 1, ` + tconf["h_id"] + `,0`)
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_adjustment/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
		...
		arr_raw_data := datatables.Data_row(`LAEDTL_SaveEdit '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,` + r.Form["member_id"][0] + `,` + r.Form["lrdtl"][0] + `,` + r.Form["amount"][0] + `,'` + r.Form["remarks"][0] + `'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/loan_adjustment/details/DDeleteHandler"
		tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_adjustment/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`LAEDTL_SaveDelete '` + username + `',` + r.Form["h_id"][0])
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	arr_raw_data := datatables.DataList(`LAEHDR_Post '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`GOOD TO POST`)
	arr_raw_data := datatables.DataList(`LAEHDR_Cancel '` + username + `',` + hdr_id)
	fmt.Println(arr_raw_data)
...
func Download_LoanBalHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arrdata := datatables.DataList(`exec LRDTL_Get 5,` + member + `,` + hdr_id)
		fmt.Println(arrdata)
		html_filename = "download_LoanBal.html"
		html_file = "transaction/loan_adjustment/download_LoanBal.html"
...
func Duplicate(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`LAEHDR_Duplicate  '` + username + `' ,  ` + tconf["id"])
	arr_data := datatables.Data_row(`LAEHDR_Duplicate  '` + username + `' ,  ` + tconf["id"])
	js, err := json.Marshal(arr_data)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
