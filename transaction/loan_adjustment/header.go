...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Status", "Trandate", "LAEType", "Remarks"} //column header
	if ln == "" {
		// content access here
		...
		var ajaxURLdata = "/transaction/loan_adjustment?rights=" + rights
		type TblConf struct {
			Headercol []string
			AjaxUrl   string
		...
		tconf["ActionCol_edit_url"] = "/transaction/loan_adjustment/HEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_view"] = "false"                                                                     // config for edit click
		tconf["ActionCol_view_is_modal"] = "false"                                                            // config for edit click
		//tconf["ActionCol_edit_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click
		tconf["ActionCol_view_url"] = "/transaction/loan_adjustment/HViewHandler?rights=" + rights + "&h_id=" // config for edit click

		tconf["ActionCol_delete"] = "false" // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/loan_adjustment/HDeleteHandler?h_id="
		tconf["ActionCol_detail"] = "true"     // config for details click
		tconf["ActionCol_add_child"] = "false" // config for addchild click
		tconf["ActionCol_loanconsolidation_url"] = "/transaction/loan_adjustment/details/DListHandler?h_id="
		tconf["style_size"] = "12"
		tconf["style_font"] = ""
		...
		tconf["sql_total"] = `LAEHDR_List 1, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `LAEHDR_List 0, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data_laetype := datatables.DataList(`exec LAEType_Get 1, 0`)
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_adjustment/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec LAEHDR_Get 1,` + tconf["h_id"])
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/loan_adjustment/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_laetype})
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/loan_adjustment/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`LAEHDR_SaveDelete '` + username + `' , ` + r.Form["h_id"][0] + ``)
		/*
			fmt.Println(arr_data)
