...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Other Disbursement"
	tconf["Add_new_button"] = "false"
	tconf["Rights"] = rights
	tconf["Add_back_button"] = "false"
	tconf["Add_new_button_details"] = "false"
	tconf["upload_button"] = "false"
	tconf["Add_new_button_url"] = "/transaction/other_disbursement/HaddHandler?rights=" + rights
	...
	var header = []string{"Action", "ID", "Status", "Trandate", "Transaction Type", "Partner", "Remarks"} //column header
	if ln == "" {
		// content access here
		...
		var ajaxURLdata = "/transaction/other_disbursement?rights=" + rights
		type TblConf struct {
			Headercol []string
			AjaxUrl   string
		...
		tconf["ActionCol_edit_url"] = "/transaction/other_disbursement/HEditHandler?rights=" + rights + "&h_id=" // config for edit click
		tconf["ActionCol_view"] = "false"                                                                             // config for edit click
		tconf["ActionCol_view_is_modal"] = "false"                                                                    // config for edit click
		//tconf["ActionCol_edit_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click
		tconf["ActionCol_view_url"] = "/transaction/other_disbursement/HViewHandler?rights=" + rights + "&h_id=" // config for edit click

		tconf["ActionCol_delete"] = "false" // config for delete click
		tconf["ActionCol_delete_url"] = "/transaction/other_disbursement/HDeleteHandler?h_id="
		tconf["ActionCol_detail"] = "true"     // config for details click
		tconf["ActionCol_add_child"] = "false" // config for addchild click
		tconf["ActionCol_loanconsolidation_url"] = "/transaction/other_disbursement/details/DListHandler?h_id="
		tconf["style_size"] = "12"
		tconf["style_font"] = ""
		...
		tconf["sql_total"] = `OtherDisHdr_List 1, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `OtherDisHdr_List 0, ` + str_OrgID + `,  ` + str + `,  ` + ln + `, '` + sr + `',` + archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_date_transactiontype := datatables.DataList(`exec OthDisHdr_trantype_List 0`)
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/other_disbursement/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf, arr_data_partner, arr_date_transactiontype })
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec OtherDisHdr_Get ` + tconf["h_id"])
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("transaction/other_disbursement/Hedit.html"); err != nil {
			fmt.Println(err)
		}
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/transaction/other_disbursement/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`OtherDisHdr_SaveDelete '` + username + `' , ` + r.Form["h_id"][0] + ``)
		/*
			fmt.Println(arr_data)
