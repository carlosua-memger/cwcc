...
func LBR_OTHdr_ListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Overtime Logs" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderCreate?rights="+rights
	tconf["Action_baseURL"] ="/timekeeping/overtime_logs/details?rights="+rights
	...
	var header = []string{"Action", "ID", "Transaction Date","Status","Assignment Name","Remarks"}  //column header
	if ln=="" {
		...
		var ajaxURLdata = "/timekeeping/overtime_logs?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_delete_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderDelete?h_id="
...
func do_LBR_OTHdr_CreateHandler(trandate string, lbr_assign string, remarks string , username string) (bool ,int) {
...
func LBR_OTHdr_CreateHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	...
	if r.Method =="GET" {	
		// new struct for Org
		rows2 ,err, _,_ := config.Ap_sql("LBR_Assign_Get 2, 2, "+str_ses_uID,1)
	    if err != nil {
	    	panic(err.Error())
	    }
		type row2 struct {
		...
		type Data struct {
     	...
     	tmpl := template.New("OvertimelogsHeaderCreate.html")
		//var err error
		if tmpl, err = tmpl.ParseFiles("timekeeping/overtime_logs/OvertimelogsHeaderCreate.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
			...
			add_status, lastinsertedID :=  do_LBR_OTHdr_CreateHandler(r.Form["trandate"][0],r.Form["lbr_assign"][0],r.Form["remarks"][0],username)
...
func LBR_OTHdr_Get_id( Hdr_id string ) LBR_OTHdr {
    ...
    var r LBR_OTHdr
    for db_raw.Next() {
        err = db_raw.Scan(&r.ID, &r.Status,&r.Trandate,&r.Lbr_assign,&r.Remarks)
        if err != nil {
            panic(err.Error())
        }
	}
...
func LBR_OTHdr_EditHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	rights :=r.URL.Query().Get("rights")
	if r.Method =="GET" {	
		tconf := make(map[string]string)
		...
		type Data struct {
	 	...
	 	tmpl := template.New("OvertimelogsHeaderEdit.html")
		//var err error
		if tmpl, err = tmpl.ParseFiles("timekeeping/overtime_logs/OvertimelogsHeaderEdit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{got2,tconf,LBR_OTHdr_Get_id(tconf["h_id"])} )
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
	}else{
...
func LBR_OTHdr_DeleteHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/timekeeping/overtime_logs/OvertimeLogsHeaderDelete"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("timekeeping/overtime_logs/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
