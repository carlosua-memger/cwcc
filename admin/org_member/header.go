...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Org Member Record" 
	tconf["Add_new_button"] ="true" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/administrator/org_member/HaddHandler?rights="+rights 
	tconf["Action_baseURL"] ="/administrator/org_member?rights="+rights
	...
	var header = []string{"Action", "ID", "MemberID", "Status", "Join date", "Member Name"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/administrator/org_member?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("admin/org_member/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] ="/administrator/org_member/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="true" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/administrator/org_member/HDeleteHandler?h_id="
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("admin/org_member/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("admin/org_member/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/administrator/org_member/HDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("admin/org_member/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		arr_data := datatables.Data_row(`OrgMem_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
		/*
		fmt.Println(arr_data) 
