...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Member Role Record" 
	tconf["Add_new_button"] ="true" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/administrator/member_role/HaddHandler?rights="+rights 
	tconf["Action_baseURL"] ="/administrator/member_role?rights="+rights
	...
	var header = []string{"Action", "ID", "Role Name", "Member Name"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/administrator/member_role?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("admin/member_role/table.html"); err != nil {
			fmt.Println(err)
		}
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		fmt.Println(str_OrgID)
		tconf["sql_total"] = `SysRolAcc_List 1, ,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SysRolAcc_List 0, ,  `+str+`,  `+ln+`, '`+sr+`'`
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("admin/member_role/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
