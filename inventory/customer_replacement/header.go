...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Customer Replacement" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/customer_replacement/HaddHandler?rights="+rights 
	...
	if ln=="" {
		...
		var ajaxURLdata = "/inventory/customer_replacement?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_replacement/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_replacement/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_CusRepHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		
		tconf["sql_data"]  = `SIS_CusRepHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_location,arr_data_customer, arr_data_tag})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`exec SIS_CusRepHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`)
		arr_data := datatables.Data_row(`exec SIS_CusRepHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		
		fmt.Println(arr_data) 
		tconf["cust_rep_id"] = 				arr_data[0]
		tconf["cust_rep_status"] = 			arr_data[1] 
		tconf["cust_rep_location"] = 		arr_data[2] 
		tconf["cust_rep_location_name"] = 	arr_data[3]
		tconf["cust_rep_trndate"] = 		arr_data[4] 
		tconf["cust_rep_customer_name"] = 	arr_data[5] 
		tconf["cust_rep_remarks"] = 		arr_data[6]
		tconf["cust_rep_customer"] = 		arr_data[7] 
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/Hedit.html"); err != nil {
			fmt.Println(err)
		}
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_replacement/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_CusRepHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
