...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Customer Replacement Detail" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="true"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	//tconf["Add_new_button_url"] ="/inventory/customer_replacement/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Action_baseURL"] ="/inventory/customer_replacement/details?rights="+rights
	tconf["ActionCol_param"] ="ID"
    //tconf["hdr_id"] = ""
    tconf["Archive"] ="false" // is archive
	tconf["Interpret_button"] ="false"
	tconf["Add_post_button"] ="false"
	tconf["Add_post_button_url"] ="/inventory/customer_replacement/details/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/customer_replacement/details/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Add_upload_button"] ="false"
	tconf["Add_upload_button_url"] ="/inventory/customer_replacement/HaddHandler?rights="+rights+"&fromtype=cp&cp_id="+tconf["hdr_id"] 
	tconf["Add_duplicate_button"] ="false"
	tconf["Add_duplicate_button_url"] ="/inventory/customer_replacement/details/duplicate_sales?rights="+rights+"&fromtype=cp&cp_id="+tconf["hdr_id"] 
	...
	var header = []string{"Action", "Item Name", "Qty", "Unit Charge", "Total Price","Remarks"}  //column header
	if ln=="" {
		
		tconf["ismoreinfo"] = "true" // to description info
		fmt.Println(`exec SIS_CusRepHdr_Get 1, `+str_OrgID+`, '`+tconf["hdr_id"]+`'`   )
		arr_data := datatables.Data_row( `exec SIS_CusRepHdr_Get 1, `+str_OrgID+`, '`+tconf["hdr_id"]+`'`  )
		...
		tconf["Add_new_button_url"] ="/inventory/customer_replacement/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["cust_ret_customer"]+"&location="+tconf["cust_ret_location"]
		tconf["Download_CusRet_url"] ="/inventory/customer_replacement/details/DownloadCusRet?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["cust_ret_customer"]+"&location="+tconf["cust_ret_location"]
		tconf["Download_SupRep_url"] ="/inventory/customer_replacement/details/DownloadSupRep?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["cust_ret_customer"]+"&location="+tconf["cust_ret_location"]
		// content access here
		// rights for tables 	
		...
		var ajaxURLdata = "/inventory/customer_replacement/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["cust_ret_status"]+"&location="+tconf["cust_ret_location"]
		fmt.Println("/inventory/customer_replacement/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["cust_ret_status"]+"&location="+tconf["cust_ret_location"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_replacement/details/DEditHandler?rights="+rights+"&location="+location+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_replacement/details/DDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_CusRepDtl_List 1, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_CusRepDtl_List 0, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["org"] = str_OrgID
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/customer_replacement/Dadd_barcode.html"
		}else{
			html_filename = "Dadd.html"
			html_file = "inventory/customer_replacement/Dadd.html"
		}
		...
		fmt.Println(`SIS_CusRepDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_charge"][0]+`,'`+r.Form["remarks"][0]+`'`)
		arr_raw_data := datatables.Data_row(`SIS_CusRepDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_charge"][0]+`,'`+r.Form["remarks"][0]+`'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`exec SIS_CusRepDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		arr_data := datatables.Data_row(`exec SIS_CusRepDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		
		tconf["custrep_detail_id"] = 		arr_data[0]
		tconf["custrep_detail_item"] = 		arr_data[1] 
		tconf["custret_detail_item_name"] = arr_data[2] 
		tconf["custret_detail_qty"] = 		arr_data[3]
		tconf["custret_detail_ucharge"] = 	arr_data[4] 
		tconf["custret_detail_tcharge"] = 	arr_data[5]
		tconf["custret_detail_remarks"] = 	arr_data[6]
		tconf["custret_detail_hdr_id"] = 	arr_data[7]
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		...
		fmt.Println(`SIS_CusRepDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_charge"][0]+`,'`+r.Form["remarks"][0]+`'`)
		arr_raw_data := datatables.Data_row(`SIS_CusRepDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_charge"][0]+`,'`+r.Form["remarks"][0]+`'`)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_replacement/details/DDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_CusRepDtl_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
	...
	getq = getq + getid
		...
		str_OrgID :=strconv.Itoa(Org_id)
		 if (isbaseonassignid ==`1`){
		 	...
		 	sqlstr ="exec SIS_CusRep_Misc 2, "+str_OrgID+",0, "+hdr_id+",'"+getq+"'"
		 }else{
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
		...
		arr_raw_data := datatables.DataList(`SIS_CusRep_Post_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_CusRep_Post '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
		...
		arr_raw_data := datatables.DataList(`SIS_CusRep_Cancel_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_CusRep_Cancel '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DAdd_reference(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/Dadd_reference.html"); err != nil {
			fmt.Println(err)
		}
		err = tmpl.Execute(w,tconf)
		if err != nil {
...
func Show_prev_price(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if tmpl, err = tmpl.ParseFiles("inventory/customer_replacement/show_prev.html"); err != nil {
		fmt.Println(err)
	}
...
func Download_CusRetHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arrdata := datatables.DataList(`SIS_CusRepHdr_Misc 1,`+hdr_id)
		fmt.Println(arrdata);
		html_filename = "download_CusRet.html"
		html_file = "inventory/customer_replacement/download_CusRet.html"
		...
		ret_add := datatables.Data_row(`SIS_CusRepDtl_DownLoad  1, '`+username+`', `+hrd_id+`, `+checkbox_id+`,`+qty )
...
func Download_SupRepHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	username, _ := login.Get_account_info(r)
	Org_id :=login.Get_session_org_id(r)
	str_OrgID :=strconv.Itoa(Org_id)
	...
	if r.Method =="GET" {	
		...
		arrdata := datatables.DataList(`SIS_CusRepHdr_Misc 2,`+hdr_id)
		fmt.Println(arrdata);
		html_filename = "download_SupRep.html"
		html_file = "inventory/customer_replacement/download_SupRep.html"
		...
		ret_add := datatables.Data_row(`SIS_CusRepDtl_DownLoad  2, '`+username+`', `+hrd_id+`, `+checkbox_id+`,`+qty )
