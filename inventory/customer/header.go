...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Customer Master List" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/customer/HaddHandler?rights="+rights 
	//tconf["Action_baseURL"] ="/master/branch?rights="+rights
	tconf["Action_baseURL"] ="/inventory/customer/location?rights="+rights //application link 	
	...
	if ln=="" {
		...
		var ajaxURLdata = "/inventory/customer?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] ="/inventory/customer/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer/HDeleteHandler?h_id="
	 	...
	 	tconf["ActionCol_location"] ="true" // config for application click 
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
		...
		tconf["sql_total"] = `SIS_Customer_List 1, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_Customer_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights,tconf,arr_data_itemtag,arr_data_courier,arr_data_orgmem})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_Customer_Get 1, '`+str_OrgID+`', '`+tconf["h_id"]+`'`  )
		...
		tconf["item_creditlimit"] = arr_data[9] 
		tconf["item_courier"] = 	arr_data[10]
		tconf["item_couriername"] = arr_data[11]
		tconf["item_remarks"] = 	arr_data[12] 
		tconf["item_member"] = 		arr_data[13] 
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer/Hedit.html"); err != nil {
			fmt.Println(err)
		}
