...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Location Name", "Status", "Trans Date", "Supplier Name","Term", "Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/supplier_receipt?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/supplier_receipt/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/supplier_receipt/HDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_SupRecHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_SupRecHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data_tag := datatables.DataList(`exec SIS_SupRecTags_List 1,`+str_OrgID+` , 0`  )
		type Data struct {
			Rights	string	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_receipt/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HAddTagHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
				fmt.Println(`SIS_SupRecTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
	        	arr_data_itemtag := datatables.Data_row(`SIS_SupRecTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_SupRecHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		arr_data_location := datatables.DataList(`exec SIS_Location_Get 5,`+str_OrgID  )
		arr_data_supplier := datatables.DataList(`SIS_Supplier_Get 2,`+str_OrgID  )
		arr_data_tag := datatables.DataList(`SIS_SupRecTags_List 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		

		tmpl := template.New("Hedit.html").Funcs(local_FuncMap) 
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_receipt/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location,arr_data_supplier,arr_data_tag} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/supplier_receipt/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_SupRecHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
