...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Supplier Receipt Detail" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="true"
	tconf["Add_new_button_details"] ="true" 
	tconf["upload_button"] ="false"   
	//tconf["Add_new_button_url"] ="/inventory/supplier_receipt/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Action_baseURL"] ="/inventory/supplier_receipt/details?rights="+rights
	tconf["ActionCol_param"] ="ID"
    //tconf["hdr_id"] = ""
	tconf["Interpret_button"] ="false"
	tconf["Add_post_button"] ="false"
	tconf["Add_post_button_url"] ="/inventory/supplier_receipt/details/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/supplier_receipt/details/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	...
	var header = []string{"Action", "Item Name", "Qty", "Unit Cost", "Disc 1", "Disc 2", "Disc 3", "Discount Amount", "Discount Total", "Net", "Remarks"}  //column header
	if ln=="" {
		
		tconf["ismoreinfo"] = "true" // to description info
		arr_data := datatables.Data_row(`SIS_SupRecHdr_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'`  )
		...
		tconf["Add_new_button_url"] ="/inventory/supplier_receipt/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&location="+tconf["suprec_hdr_location"]+"&supplier="+tconf["suprec_hdr_supplier"]
		tconf["Download_SupOrd_url"] ="/inventory/supplier_receipt/details/DownloadSupOrd?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&location="+tconf["suprec_hdr_location"]+"&supplier="+tconf["suprec_hdr_supplier"]
		// content access here
		// rights for tables 	
		...
		var ajaxURLdata = "/inventory/supplier_receipt/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]
		fmt.Println("/inventory/supplier_receipt/details?rights="+rights+"&hdr_id="+tconf["hdr_id"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_receipt/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/supplier_receipt/details/DEditHandler?rights="+rights+"&location="+location+"&supplier="+supplier+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/supplier_receipt/details/DDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_SupRecDtl_List 1, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_SupRecDtl_List 0, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
	...
	if r.Method =="GET" {	
		...
		*/
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/supplier_receipt/Dadd_barcode.html"
		}else{
			html_filename = "Dadd.html"
			html_file = "inventory/supplier_receipt/Dadd.html"
		}
		...
		r.ParseForm()
		fmt.Println(`SIS_SupRecDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,`+r.Form["unit_cost"][0]+`,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+`,`+r.Form["discount_amt"][0]+`,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_SupRecDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,`+r.Form["unit_cost"][0]+`,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+`,`+r.Form["discount_amt"][0]+`,'`+r.Form["remarks"][0]+`'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`exec SIS_SupRecDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'` )
		arr_data := datatables.Data_row(`exec SIS_SupRecDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		tconf["suprec_detail_disc_per1"] = 		arr_data[5] 
		tconf["suprec_detail_disc_per2"] = 		arr_data[6]  
		tconf["suprec_detail_disc_per3"] = 		arr_data[7]  
		tconf["suprec_detail_disc_per_amnt"] = 	arr_data[8]
		tconf["suprec_detail_disc_per_total"] = arr_data[9]  
		tconf["suprec_detail_ucost"] = 			arr_data[10]
		tconf["suprec_detail_discounted_amt"] = arr_data[11] 
		tconf["suprec_detail_remarks"] = 		arr_data[12]  
		tconf["suprec_detail_hdr_id"] = 		arr_data[13]  
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_receipt/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
		...
		fmt.Println(`SIS_SupRecDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,`+r.Form["unit_cost"][0]+`,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+`,`+r.Form["discount_amt"][0]+`,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_SupRecDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,`+r.Form["unit_cost"][0]+`,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+`,`+r.Form["discount_amt"][0]+`,'`+r.Form["remarks"][0]+`'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
	    	http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/supplier_receipt/details/DDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_receipt/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_SupRecDtl_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
	...
	getq = getq + getid
		...
		str_OrgID :=strconv.Itoa(Org_id)
		 if (isbaseonassignid ==`1`){
		 	...
		 	sqlstr ="exec SIS_SupRec_Misc 2, "+str_OrgID+",0, "+hdr_id+",'"+getq+"'"
		 }else{
	    ...
	    for rows.Next() {
	        var r Inner_item
	        err = rows.Scan(&r.Id,&r.Fullname,&r.Item_image,&r.Disc_per ,&r.Disc_per2 ,&r.Disc_per3 ,&r.Disc_per_amt)
	        if err != nil {
	            panic(err.Error())
	        }
			count = count +1
	    ...
	    InItemRow = Inner_item{r.Id,r.Item_image, config.Img_to_render(`img_uploads/size_80/`+str_OrgID+`/`+r.Item_image+`.jpg`,r.Item_image,80,str_OrgID),r.Disc_per ,r.Disc_per2 ,r.Disc_per3 ,r.Disc_per_amt,}
	    }
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
		...
		username, _ := login.Get_account_info(r)
		r.ParseForm()
		fmt.Println(`SIS_SupRec_Post_Check `+str_OrgID+`,`+hdr_id )
		arr_raw_data := datatables.DataList(`SIS_SupRec_Post_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_SupRec_Post '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
		...
		username, _ := login.Get_account_info(r)
		r.ParseForm()
		fmt.Println(`SIS_SupRec_Cancel_Check `+str_OrgID+`,`+hdr_id )
		//arr_data_tag := datatables.DataList(`SIS_SupRecTags_List 1,`+str_OrgID+` ,`+ tconf["hdr_id"] )

		arr_raw_data := datatables.DataList(`SIS_SupRec_Cancel_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_SupRec_Cancel '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func Show_prev_price(w http.ResponseWriter, r *http.Request) {
    ...
    d := DData{
    	datatables.DataList(`SIS_SupRec_ShowLastRecords 2, `+str_OrgID+`, `+supplier+`, `+item+`, `+location+``) ,
    	datatables.DataList(`SIS_Item_Get 9, `+str_OrgID+`, `+item+`, `+location+``) }
    fmt.Println(d)
	tmpl := template.New("show_bal.html").Funcs(config.FuncMap)
	var err error
	if tmpl, err = tmpl.ParseFiles("helper/inven_show_balance/show_bal.html"); err != nil {
		fmt.Println(err)
	}
	err1 := tmpl.Execute(w,d)
	if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
	}
}

func Show_balance(w http.ResponseWriter, r *http.Request) {
	Org_id :=login.Get_session_org_id(r)
	str_OrgID :=strconv.Itoa(Org_id)
	supplier :=r.URL.Query().Get("supplier")
	location :=r.URL.Query().Get("location")
	...
	sql := `SIS_Item_Get 9, `+str_OrgID+`, `+item+`, `+location+``
	fmt.Println(sql)
...
func Download_SupOrdHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	username, _ := login.Get_account_info(r)
	Org_id :=login.Get_session_org_id(r)
	str_OrgID :=strconv.Itoa(Org_id)
	...
	if r.Method =="GET" {	
		...
		arrdata := datatables.DataList(`SIS_SupRecHdr_Misc 1,`+hdr_id)
		fmt.Println(arrdata);
		html_filename = "download_SupOrd.html"
		html_file = "inventory/supplier_receipt/download_SupOrd.html"
		...
		ret_add := datatables.Data_row(`SIS_SupRecDtl_DownLoad  1, '`+username+`', `+hrd_id+`, `+checkbox_id+`,`+qty )
