...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Location Name", "Status",  "Trans Date", "Customer Name","Application Balance","Application Status","Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/customer_return?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_return/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_return/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_CusRetHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		
		tconf["sql_data"]  = `SIS_CusRetHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data_supplier := datatables.DataList(`SIS_Supplier_Get 3,`+str_OrgID  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_return/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`exec SIS_CusRetHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`)
		arr_data := datatables.Data_row(`exec SIS_CusRetHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_return/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location,nil,arr_data_customer,nil, nil, arr_data_tag} )
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
	}else{
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_return/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_CusRetHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
