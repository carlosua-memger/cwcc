...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Customer Adjustment" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/customer_adjustment/HaddHandler?rights="+rights 
	...
	var header = []string{"Action", "ID", "Customer Name", "Status", "Trandate", "Type", "Amount","Application Balance","Application Status", "Remarks"}  //column header
	if ln=="" {
		...
		var ajaxURLdata = "/inventory/customer_adjustment?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_adjustment/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_adjustment/HDeleteHandler?h_id="
	 	tconf["ActionCol_detail"] ="false" // config for details click 
	 	tconf["ActionCol_add_child"] ="false" // config for addchild click 
	 	tconf["ActionCol_post"] ="true" // config for details click 
	 	tconf["ActionCol_post_url"] ="/inventory/customer_adjustment/HPostHandler?h_id="
	 	tconf["ActionCol_application"] ="true" // config for application click 
	 	//tconf["ActionCol_application_url"] ="/inventory/customer_adjustment/HApplicationHandler?h_id="
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
		...
		tconf["sql_total"] = `SIS_CusAdjHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		
		tconf["sql_data"]  = `SIS_CusAdjHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		if(fromtype==`co`){
			...
			fmt.Println(`SIS_CusOrdHdr_Get 1,`+str_OrgID+`,`+co_id )
			arr_data_co := datatables.Data_row(`SIS_CusOrdHdr_Get 2,`+str_OrgID+`,`+co_id )
			tconf["co_id"] = 				arr_data_co[0]
			tconf["co_status"] = 			arr_data_co[1] 
			tconf["co_location_id"] = 		arr_data_co[2] 
			tconf["co_location_name"] = 	arr_data_co[3]
			tconf["co_trandate"] = 			arr_data_co[4] 
			tconf["co_customer_name"] = 	arr_data_co[5] 
			tconf["co_soldby_name"] = 		arr_data_co[6]
			tconf["co_remarks"] = 			arr_data_co[7] 
			tconf["co_customer_id"] = 	 	arr_data_co[8]
			tconf["co_soldby_id"] = 		arr_data_co[9] 
			tconf["co_courier_id"] = 		arr_data_co[10] 
			tconf["co_courier_name"] = 		arr_data_co[11] 
			
		}
		fmt.Println(`-------------end testing print`)


		arr_data_location := datatables.DataList(`exec SIS_Location_Get 7,`+str_OrgID  )
		...
		//arr_data_courier := datatables.DataList(`exec SIS_Courier_Get 2,`+str_OrgID  )
		arr_data_tag := datatables.DataList(`exec SIS_CusSalTags_List 1,`+str_OrgID+` , 0`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_adjustment/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_location,arr_data_customer,arr_data_orgmem,arr_data_adjtype, arr_data_tag})
		//err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_supplier,arr_data_customer,arr_data_orgmem,arr_data_adjtype, arr_data_tag})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_CusAdjHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		
		fmt.Println(arr_data) 
		tconf["suprec_id"] = 				arr_data[0]
		tconf["suprec_status"] = 			arr_data[1] 
		tconf["suprec_transdate"] = 		arr_data[2] 
		tconf["suprec_customer"] = 			arr_data[3] 
		tconf["suprec_customer_name"] = 	arr_data[4]
		tconf["suprec_cusadjtype"] = 		arr_data[5] 
		tconf["suprec_Adjtype"] = 			arr_data[6]
		tconf["suprec_amt"] = 				arr_data[7] 
		tconf["suprec_remarks"] = 	 		arr_data[8]
		...
		arr_data_orgmem := datatables.DataList(`exec OrgMem_Get 1,`+str_OrgID+` , 0`  )
		arr_data_courier := datatables.DataList(`exec SIS_Courier_Get 2,`+str_OrgID  )
		arr_data_tag := datatables.DataList(`SIS_CusSalTags_List 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_adjustment/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location,arr_data_supplier,arr_data_customer,arr_data_orgmem, arr_data_courier, arr_data_adjtype, arr_data_tag} )
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
	}else{
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_adjustment/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_CusAdjHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
...
func HPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["post_url"] = "/inventory/customer_adjustment/HPostHandler"
	   	tmpl := template.New("modal_post_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_adjustment/modal_post_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		err = tmpl.Execute(w,tconf)
		if err != nil {
		...
		//_ ,err, _,_ := config.Ap_sql(`Supplier_Save 'Delete','programmer',1, ` +r.Form["h_id"][0] ,3)
		//_ ,err, _,_ := config.Ap_sql(`SIS_ItemClass_Save 'Delete','programmer',` +r.Form["h_id"][0]  ,3)
		arr_data := datatables.Data_row(`SIS_CusAdj_Post '`+username+`'  , `+str_OrgID+`,  `+r.Form["h_id"][0] +`` )
		/*
		fmt.Println(arr_data) 
		tconf["item_id"] = 			arr_data[0]
		tconf["item_org"] = 		arr_data[1] 
