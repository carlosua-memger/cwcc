...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Supplier Return Detail" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="true"
	tconf["Add_new_button_details"] ="true" 
	tconf["upload_button"] ="false"   
	//tconf["Add_new_button_url"] ="/inventory/supplier_return/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	...
	tconf["Add_post_button_url"] ="/inventory/supplier_return/details/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/supplier_return/details/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	...
	if ln=="" {
		...
		arr_data := datatables.Data_row(`SIS_SupRetHdr_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'`  )
		...
		tconf["Add_new_button_url"] ="/inventory/supplier_return/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]+"&supplier="+tconf["suprec_hdr_supplier"]
		tconf["Download_CustRet_url"] ="/inventory/supplier_return/details/DownloadCustRet?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		tconf["Download_LocRec_url"] ="/inventory/supplier_return/details/DownloadLocRec?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		tconf["Email_url"] ="/inventory/supplier_return/details/email?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		tconf["ActionCol_edit_is_modal"] ="true"
		// content access here
		// rights for tables 	
...
//	 			tconf["Add_cancel_button"] ="true"
		...
		arr_data_tag := datatables.DataList(`SIS_SupRetTags_List 1,`+str_OrgID+` ,`+ tconf["hdr_id"] )
		
		
		var ajaxURLdata = "/inventory/supplier_return/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]+"&location="+tconf["suprec_hdr_location"]+"&supplier="+tconf["suprec_hdr_supplier"]
		fmt.Println("/inventory/supplier_return/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]+"&location="+tconf["suprec_hdr_location"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/supplier_return/details/DEditHandler?rights="+rights+"&location="+location+"&supplier="+supplier+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/supplier_return/details/DDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_SupRetDtl_List 1, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_SupRetDtl_List 0, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["org"] = str_OrgID
	...
	if r.Method =="GET" {	
		...
		*/
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/supplier_return/Dadd_barcode.html"
		}else{
			html_filename = "Dadd.html"
			html_file = "inventory/supplier_return/Dadd.html"
		}
		...
		r.ParseForm()
		fmt.Println(`SIS_SupRetDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_SupRetDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,'`+r.Form["remarks"][0]+`'` )
...
func Download_CustRetHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	username, _ := login.Get_account_info(r)
	Org_id :=login.Get_session_org_id(r)
	str_OrgID :=strconv.Itoa(Org_id)
	...
	if r.Method =="GET" {	
		...
		arrdata := datatables.DataList(`SIS_SupRetHdr_Misc 1,`+hdr_id)
		fmt.Println(arrdata);
		html_filename = "download_Custret.html"
		html_file = "inventory/supplier_return/download_Custret.html"
		...
		ret_add := datatables.Data_row(`SIS_SupRetDtl_DownLoad  1, '`+username+`', `+hrd_id+`, `+checkbox_id+`,`+qty )
...
func Download_LocRecHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arrdata := datatables.DataList(`SIS_SupRetHdr_Misc 2,`+hdr_id)
		fmt.Println(arrdata);
		html_filename = "download_LocRec.html"
		html_file = "inventory/supplier_return/download_LocRec.html"
		...
		ret_add := datatables.Data_row(`SIS_SupRetDtl_DownLoad  2, '`+username+`', `+hrd_id+`, `+checkbox_id+`,`+qty )
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_SupRetDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_return/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
		...
		r.ParseForm()
		fmt.Println(`SIS_SupRetDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_SupRetDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,'`+r.Form["remarks"][0]+`'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
	    	http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/supplier_return/details/DDeleteHandler"
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_SupRetDtl_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
	...
	getq = getq + getid
		...
		str_OrgID :=strconv.Itoa(Org_id)
		 if (isbaseonassignid ==`1`){
		 	...
		 	sqlstr ="exec SIS_SupRet_Misc 1, "+str_OrgID+",0, "+hdr_id+",'"+getq+"'"
		 }else{
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
		...
		username, _ := login.Get_account_info(r)
		r.ParseForm()
		fmt.Println(`SIS_SupRet_Post_Check `+str_OrgID+`,`+hdr_id )
		arr_raw_data := datatables.DataList(`SIS_SupRet_Post_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_SupRet_Post '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
		...
		username, _ := login.Get_account_info(r)
		r.ParseForm()
		fmt.Println(`SIS_SupRet_Cancel_Check `+str_OrgID+`,`+hdr_id )
		//arr_data_tag := datatables.DataList(`SIS_SupRecTags_List 1,`+str_OrgID+` ,`+ tconf["hdr_id"] )

		arr_raw_data := datatables.DataList(`SIS_SupRet_Cancel_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_SupRet_Cancel '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func EmailHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		//arr_data := datatables.Data_row(`exec branch_get 1, 1,`+ tconf["h_id"]  )
		arr_data := datatables.Data_row(`exec SIS_SupRetHdr_Get 2, `+str_OrgID+`, `+hdr_id)
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier_return/email.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
	...
	fmt.Println(`SIS_SupRet_Custom 1,`+str_OrgID+` ,`+ hrd_id)
	SIS_SupRetTags_List := datatables.DataList(`SIS_SupRet_Custom 2,`+str_OrgID+` ,`+ hrd_id )
	SIS_SupRetHdr_Report := datatables.DataList(`SIS_SupRet_Custom 1,`+str_OrgID+` ,`+ hrd_id )
    ht_tbl:=`<table width="100%" cellpadding="15">
			...
			`;
    for _ , value := range SIS_SupRetTags_List{
    	ht_tbl+=`<tr>`;
    ...
    for _ , value2 := range SIS_SupRetHdr_Report{
    	ht_tbl+=`<tr>`;
	...
	e.Subject = org_name + ` Supplier Return (ID# ` +hrd_id+ `)`
	e.HTML = []byte(`Hi `+supplier_name+`,<br><br>Sending you list of returns, as follows;<br><br>`+ht_tbl+`<br>Additional Message: `+r.Form["message"][0]+`<br>Thank you<br>`+org_name+`<br><br>This is a system generated email. Do not reply on this email.	<br> Powered by <a href="www.memger.com"> Memger Tech Inc.</a> <i>Inventory like a <b>boss</b>... <a href="www.memger.com">try us for <b>free</b>.</a></i>`)
...
/*func EmailHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`SIS_Supplier_Get 1 ,  `+str_OrgID+` ,`+supplier)
	arr_supplier := datatables.Data_row(`SIS_Supplier_Get 1 ,  `+str_OrgID+` ,`+supplier  )
	email_to :=arr_supplier[7];
	supplier_name :=arr_supplier[2];
	...
	fmt.Println(`SIS_SupRet_Custom 1,`+str_OrgID+` ,`+ hrd_id)
	SIS_SupRetTags_List := datatables.DataList(`SIS_SupRet_Custom 2,`+str_OrgID+` ,`+ hrd_id )
    ht_tbl:=`<table width="100% " style=";border: 1px solid;">
	            <tr>
	                <th align="left" style=";border: 1px solid;"><b>Record #</th>
	                <th align="left" style=";border: 1px solid;"><b>Item</th>
	                <th align="right" style=";border: 1px solid;"><b>Qty</th>
	                <th align="right" style=";border: 1px solid;"><b>Remarks</th>
	            </tr>        
			`;
    for _ , value := range SIS_SupRetTags_List{
    	ht_tbl+=`<tr>`;
	    	ht_tbl+=`<td style=";border: 1px solid;">`+value[0]+`</td>`;
	    	ht_tbl+=`<td style=";border: 1px solid;">`+value[1]+`</td>`;
	    	ht_tbl+=`<td style=";border: 1px solid;">`+value[2]+`</td>`;
	    	ht_tbl+=`<td style=";border: 1px solid;">`+value[3]+`</td>`;
    	ht_tbl+=`</tr>`;
    }
	ht_tbl+=`</table>`
	...
	fmt.Println("Sending email");
        ...
        config.Email_password,
         []string{email_to},
        "Item Returns from RD Electronics",
        `
			...
			Here are the list of items ready for your pickup.
			<br>
			<br>
			...
			<br>
			<br>
			This is a system generated email. Do not reply on this email.
			<br>
