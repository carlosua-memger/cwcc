...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	//tconf["Panel_name"] ="Ledger Report" 
	...
	var header = []string{"","Location", "Item Name", "Trans Date", "Quantity", "Unit Cost", "Balance", "Total Cost"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/ledger_asof?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
			Tempconf map[string]string
		}
		tmpl := template.New("ledger_asof.html")
		var err error
		//if tmpl, err = tmpl.ParseFiles("hris/assignments/assignment_list.html"); err != nil {
		if tmpl, err = tmpl.ParseFiles("inventory/ledger_asof/ledger_asof.html"); err != nil {
			fmt.Println(err)
		}
		...
		//tconf["ActionCol"] ="false" // config for action column
	 	...
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
	 	tconf["format_number_col"] ="Total Cost"
	 	tconf["col_img"] ="false"
		...
		tconf["sql_total"] = `SIS_Ledger_Report_period 1, `+str_OrgID+`,'06/30/2016','08/31/2016',0,0,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_Ledger_Report_period 0, `+str_OrgID+`,'06/30/2016','08/31/2016',0,0,  `+str+`,  `+ln+`, '`+sr+`'`
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_location,arr_data_supplier,arr_data_orgmem, arr_data_tag})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
	...
	getq = getq + getid
	    ...
	    for rows.Next() {
			...
			post := Inner_item {
	        	r.Id,
	        	r.Fullname, 
	        	config.Img_to_render(`img_uploads/size_80/`+r.Id+`.jpg`,r.Id,80), 
		    }
		    InItem = append(InItem, post)
	    InItemRow = Inner_item{r.Id,r.Fullname, config.Img_to_render(`img_uploads/size_80/`+r.Id+`.jpg`,r.Id,80)}
	    }
