...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Transaction Type", "Transaction ID", "Transaction Date", "Transaction Amount", "Applied Amount", "Transaction Balance"}  //column header
	if ln=="" {
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`SIS_CusAccApp_Save 'Add','`+username+`',0, `+r.Form["cusapp"][0]+`,`+r.Form["cussal"][0]+` ,`+r.Form["amount"][0]+` `)
		arr_raw_data := datatables.Data_row(`SIS_CusAccApp_Save 'Add','`+username+`',0, `+r.Form["cusapp"][0]+`,`+r.Form["cussal"][0]+` ,`+r.Form["amount"][0]+` `)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`SIS_CusAccApp_Save 'Edit','`+username+`',`+r.Form["id"][0]+`, `+r.Form["cusapp"][0]+`,`+r.Form["cussal"][0]+` ,`+r.Form["amount"][0]+` `)
		arr_raw_data := datatables.Data_row(`SIS_CusAccApp_Save 'Edit','`+username+`',`+r.Form["id"][0]+`, `+r.Form["cusapp"][0]+`,`+r.Form["cussal"][0]+` ,`+r.Form["amount"][0]+` `)
