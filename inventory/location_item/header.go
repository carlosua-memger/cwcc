...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Location Items" 
	tconf["Add_new_button"] ="true" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/location_item/HaddHandler?rights="+rights 
	tconf["Action_baseURL"] ="/inventory/location_item?rights="+rights
	...
	if ln=="" {
		...
		var ajaxURLdata = "/inventory/location_item?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/location_item/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] ="/inventory/location_item/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="true" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/location_item/HDeleteHandler?h_id="
	 	...
	 	tconf["ActionCol_duplicate_url"] ="/inventory/location_item/HDuplicateHandler?rights="+rights+"&h_id="
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_item/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		tconf["comm_per"] = 			arr_data[12] 
		tconf["comm_amt"] = 			arr_data[13] 
		tconf["tax_per"] = 				arr_data[14] 
		tconf["tax_amt"] = 				arr_data[15] 
		tconf["ads_per"] = 				arr_data[16] 
		tconf["ads_amt"] = 				arr_data[17] 
		tconf["freight_per"] = 			arr_data[18] 
		tconf["freight_amt"] = 			arr_data[19] 
...
func HDuplicateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		tconf["item_image"] = 			helper.Img_to_render(`img_uploads/size_171/`+arr_data[1]+`/`+arr_data[2]+`.jpg` ,arr_data[2],arr_data[1])
		tconf["unit_price"] = 			arr_data[9] 
		tconf["unit_cost"] = 			arr_data[10] 
		tconf["sku"] = 					arr_data[11] 
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/item/Hduplicate.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_itemclass,arr_data_supplier,arr_data_itemtag} )
		if err1 != nil {
