...
func AListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Action_baseURL"] ="/inventory/internal_order/details?rights="+rights
	...
	tconf["Add_post_button_url"] ="/inventory/internal_order/details/allocation/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/internal_order/details/allocation/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	...
	if ln=="" {
		...
		fmt.Println(`exec SIS_IntOrdDtl_Get 1, `+str_OrgID+`, '`+tconf["hdr_id"]+`'`  )
		arr_data := datatables.Data_row(`exec SIS_IntOrdDtl_Get 1, `+str_OrgID+`, '`+tconf["hdr_id"]+`'`  )
		...
		tconf["Add_new_button_url"] ="/inventory/internal_order/details/allocation/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&d_id="+tconf["suprec_detail_hdr_id"]
		// content access here		
		// rights for tables 	
		...
		var ajaxURLdata = "/inventory/internal_order/details/allocation?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&d_id="+tconf["suprec_detail_hdr_id"]+"&status="+status
		fmt.Println("/inventory/internal_order/details/allocation?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&d_id="+tconf["suprec_detail_hdr_id"]+"&status="+status)
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/internal_order/details/allocation/DEditHandler?rights="+rights+"&d_id="+d_id+"&hdr_id="+hdr_id+"&h_id=" // config for edit click 
	 	tconf["ActionCol_edit_is_modal"] ="true" // config for edit click  modal design
	 	tconf["ActionCol_delete"] ="true" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/internal_order/details/allocation/DDeleteHandler?h_id="
		...
		//SP SIS_IntAcc_List 1|0, ‘IntOrd’, @headerID, @detailID, @pageno, @rowperpage, @searchkey

		tconf["sql_total"] = `SIS_IntAcc_List 1, 'IntOrd', `+d_id+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_IntAcc_List 0, 'IntOrd', `+d_id+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["org"] = str_OrgID
...
func AAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arrdata:= datatables.DataList(`exec SIS_IntAct_Get 2,`+str_OrgID+`, 'IntOrd', '`+tconf["d_id"]+`' , '`+tconf["hdr_id"]+`' `   )
		fmt.Println(arrdata);
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/internal_order/DAllocation.html"
		}
		...
		ret_add := datatables.Data_row(`SIS_IntAcc_Save  'Add',  '`+username+`', 0, `+str_OrgID+`, 'IntOrd', `+h_id+`, `+hrd_id+`, `+checkbox_id+`,`+qty+`, '`+remarks+`'` )
...
func AEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/internal_order/Aedit.html"); err != nil {
			fmt.Println(err)
		}
		//err1 := tmpl.Execute( w,&Data{rights,tconf,arrdata} )
		...
		fmt.Println(`SIS_IntAcc_Save  'edit',  '`+username+`', `+r.Form["h_id"][0]+`, `+str_OrgID+`, 'IntOrd', 0,0, `+r.Form["leader"][0]+`, `+r.Form["qty"][0]+`, '`+r.Form["remarks"][0]+`'`)
		arr_raw_data := datatables.Data_row(`SIS_IntAcc_Save  'edit',  '`+username+`', `+r.Form["h_id"][0]+`, `+str_OrgID+`, 'IntOrd', 0,0, `+r.Form["leader"][0]+`, `+r.Form["qty"][0]+`, '`+r.Form["remarks"][0]+`'`)
...
func ADeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	...
	   	tconf["delete_url"] = "/inventory/internal_order/details/allocation/DDeleteHandler"
