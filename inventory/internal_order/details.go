...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Internal Order Detail" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="true"
	tconf["Add_new_button_details"] ="true" 
	tconf["upload_button"] ="false"   
	//tconf["Add_new_button_url"] ="/inventory/internal_order/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Action_baseURL"] ="/inventory/internal_order/details/allocation?rights="+rights
	...
	tconf["Add_post_button_url"] ="/inventory/internal_order/details/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/internal_order/details/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Add_upload_button"] ="false"
	tconf["Add_upload_button_url"] ="/inventory/internal_issuance/HaddHandler?rights="+rights+"&fromtype=co&co_id="+tconf["hdr_id"] 
	...
	if ln=="" {
		...
		arr_data := datatables.Data_row(`SIS_IntOrdHdr_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'`  )
		...
		tconf["suprec_soldby"] = 				arr_data[5] 
		tconf["suprec_hdr_soldby_name"] = 		arr_data[6]
		tconf["suprec_hdr_remarks"] = 			arr_data[7] 


		//tconf["Action_baseURL"] ="/inventory/internal_order/details/allocation?rights="+rights
		tconf["Add_new_button_url"] ="/inventory/internal_order/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_soldby"]+"&location="+tconf["suprec_hdr_location"]+"&status="+tconf["suprec_hdr_status"]
		// content access here
		// rights for tables 	
		...
		var ajaxURLdata = "/inventory/internal_order/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]+"&location="+tconf["suprec_hdr_location"]+"&hrd_id="+tconf["suprec_hdr_id"]
		fmt.Println("/inventory/internal_order/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]+"&location="+tconf["suprec_hdr_location"]+"&hrd_id="+tconf["suprec_hdr_id"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/internal_order/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/internal_order/details/DEditHandler?rights="+rights+"&location="+location+"&h_id=" // config for edit click 
	 	...
	 	tconf["ActionCol_allocation_url"] =  "/inventory/internal_order/details/allocation?rights="+rights+"&location="+location+"&status="+status+"&hdr_id="+hdr_id+"&ID=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/internal_order/details/DDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_IntOrdDtl_List 1, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_IntOrdDtl_List 0, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["org"] = str_OrgID
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/internal_order/Dadd_barcode.html"
		}else{
			html_filename = "Dadd.html"
			html_file = "inventory/internal_order/Dadd.html"
		}
		...
		fmt.Println(`SIS_IntOrdDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_cost"][0]+`,'`+r.Form["remarks"][0]+`'`)
		arr_raw_data := datatables.Data_row(`SIS_IntOrdDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_cost"][0]+`,'`+r.Form["remarks"][0]+`'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`exec SIS_IntOrdDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		arr_data := datatables.Data_row(`exec SIS_IntOrdDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/internal_order/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
		...
		r.ParseForm()
		fmt.Println(`SIS_IntOrdDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_cost"][0]+`,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_IntOrdDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_cost"][0]+`,'`+r.Form["remarks"][0]+`'` )
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/internal_order/details/DDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/internal_order/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_IntOrdDtl_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
		...
		arr_raw_data := datatables.DataList(`SIS_IntOrd_Post_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_IntOrd_Post '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
		...
		arr_raw_data := datatables.DataList(`SIS_IntOrd_Cancel_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_IntOrd_Cancel '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
