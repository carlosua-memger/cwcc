...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		*/
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/reports/non_selling_aging/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,nil,arr_data_supplier,nil,nil,nil,nil,nil})
		if err1 != nil {
...
func ViewerHandler(w http.ResponseWriter, r *http.Request) {
	...
	header := []string{ "Item Name","Supplier Name", "Ending", "Customer Sale"}  //column header	
	...
	str_page_dir := `inventory/reports/non_selling_aging/viewer.html`
...
func AjaxViewerHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`SIS_Rep_Product_salenomovement 1,`+tconf["org_id"]+`,`+tconf["item"]+`, '`+tconf["item_key"]+`', `+tconf["supplier"]+`, `+tconf["page"]+` ,50`)
	arr_data := DataList(`SIS_Rep_Product_salenomovement 1,`+tconf["org_id"]+`,`+tconf["item"]+`, '`+tconf["item_key"]+`', `+tconf["supplier"]+`, `+tconf["page"]+` ,50`, tconf["org_id"])
