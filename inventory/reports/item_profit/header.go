...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data_yrmo := datatables.DataList(`SIS_CusSalHdrArc_GetYrMo 1,`+str_OrgID)
		...
		*/
		...
		type Data struct {
			...
			Arr_item_yrmo [][]string


		}   
		tmpl := template.New("Hadd.html").Funcs(local_FuncMap) 
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/reports/item_profit/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,nil,arr_data_supplier,nil,arr_data_location,nil,nil,nil,arr_data_yrmo})
		if err1 != nil {
...
func ViewerHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["yrmo"] 	=r.URL.Query().Get("yrmo")
	tconf["location"] 	=r.URL.Query().Get("location")
	tconf["item"] 	=r.URL.Query().Get("item")
	tconf["comm"] 	=r.URL.Query().Get("comm")
	tconf["disc"] 	=r.URL.Query().Get("disc")



	header := []string{ "Item Name"," Total Quantity", "Total Price", "Total Commision", "Total After Commission", "Total Cost", "Total After Cost","Total Discount","Total After Discount",}  //column header	
	
	if(tconf["item"]==""){
		tconf["item"]="0"
	}
	
	fmt.Println(`exec SIS_Rep_Item_Profit 0, `+tconf["org_id"]+`, '`+tconf["yrmo"]+`', `+tconf["comm"]+`, `+tconf["disc"]+`, `+tconf["item"]+`, `+tconf["location"]+`, 1, 100`)
	arr_data_1 := datatables.DataList(`exec SIS_Rep_Item_Profit 0, `+tconf["org_id"]+`, '`+tconf["yrmo"]+`', `+tconf["comm"]+`, `+tconf["disc"]+`, `+tconf["item"]+`, `+tconf["location"]+`, 1, 100` )

	fmt.Println(`exec SIS_Rep_Item_Profit  2, `+tconf["org_id"]+`, '`+tconf["yrmo"]+`', `+tconf["comm"]+`, `+tconf["disc"]+`, `+tconf["item"]+`, `+tconf["location"]+`, 1, 100`)
	arr_data_2 := datatables.DataList(`exec SIS_Rep_Item_Profit 2, `+tconf["org_id"]+`, '`+tconf["yrmo"]+`', `+tconf["comm"]+`, `+tconf["disc"]+`, `+tconf["item"]+`, `+tconf["location"]+`, 1, 100` )
	...
	str_page_dir := `inventory/reports/item_profit/viewer.html`
	...
	err1 := tmpl.Execute(w,Data{tconf,header,arr_data_1,arr_data_2})
	if err1 != nil {
		http.Error(w, err1.Error(), http.StatusInternalServerError)
	}
} 
...
func AjaxViewerHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(`exec SIS_Rep_Item_Profit 1, `+tconf["org_id"]+`, '`+tconf["yrmo"]+`', `+tconf["comm"]+`, `+tconf["disc"]+`, `+tconf["item"]+`, `+tconf["location"]+`, `+tconf["page"]+`, 100`)
	arr_data := DataList(`exec SIS_Rep_Item_Profit 1, `+tconf["org_id"]+`, '`+tconf["yrmo"]+`', `+tconf["comm"]+`, `+tconf["disc"]+`, `+tconf["item"]+`, `+tconf["location"]+`, `+tconf["page"]+`, 100` , tconf["org_id"])
