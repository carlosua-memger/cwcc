...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Internal Issuance" 
	...
	tconf["Add_new_button_url"] ="/inventory/internal_issuance/HaddHandler?rights="+rights 
	...
	var header = []string{"Action", "ID", "Location Name", "Status",  "Trans Date", "Issued To","Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/internal_issuance?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/internal_issuance/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/internal_issuance/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_IntIssHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		
		tconf["sql_data"]  = `SIS_IntIssHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		if(fromtype==`co`){
			...
			fmt.Println(`SIS_IntOrdHdr_Get 1,`+str_OrgID+`,`+co_id )
			arr_data_co := datatables.Data_row(`SIS_IntOrdHdr_Get 1,`+str_OrgID+`,`+co_id )
			...
			tconf["co_orderBy"] =		 	arr_data_co[5] 
			tconf["co_orderBy_name"] = 		arr_data_co[6]
		...
		arr_data_tag := datatables.DataList(`exec SIS_IntIssTags_List 1,`+str_OrgID+` , 0`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/internal_issuance/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_location, arr_data_tag, arr_data_orgmem})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
...
func HAddTagHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
				fmt.Println(`SIS_IntIssTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_IntIssHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/internal_issuance/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location, arr_data_tag,arr_data_orgmem} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/internal_issuance/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_IntIssHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
