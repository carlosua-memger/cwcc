...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Bank Master File" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/banks/HaddHandler?rights="+rights 
	tconf["Action_baseURL"] ="/inventory/banks?rights="+rights
	...
	var header = []string{"Action", "ID", "Bank Code", "Bank Name", "Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/banks?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/banks/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] ="/inventory/banks/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/banks/HDeleteHandler?h_id="
	 	...
	 	tconf["ActionCol_duplicate_url"] ="/inventory/banks/HDuplicateHandler?rights="+rights+"&h_id="
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
		...
		if( strings.Contains(dec_rights, "HDetails") ){
			tconf["ActionCol_detail"] ="false" // config for delete click 
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_FinIns_List 1, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_FinIns_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["org"] = str_OrgID
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/banks/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_FinIns_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		arr_data_itemtag := datatables.DataList(`sis_itemtags_list 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		

		tconf["bank_id"] = 				arr_data[0]
		tconf["bank_org"] = 			arr_data[1] 
		tconf["bank_name"] = 			arr_data[2]
		tconf["bank_longname"] = 		arr_data[3]
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/banks/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_itemtag} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/banks/HDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/banks/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		arr_data := datatables.Data_row(`SIS_FinIns_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +`` )
...
func HDuplicateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/banks/Hduplicate.html"); err != nil {
			fmt.Println(err)
		}
...
func Ajax_getItem(w http.ResponseWriter, r *http.Request) {
	...
	getq = getq + getid
	    ...
	    for rows.Next() {
	        var r Inner_item
	        err = rows.Scan(&r.Id,&r.Fullname,&r.Item_image,&r.Price ,&r.Ucost,&r.Sku,&r.Comm_per,&r.Comm_amt,&r.Remarks,&r.Tax_per,&r.Tax_amt,&r.Ads_per,&r.Ads_amt,&r.Freight_per,&r.Freight_amt)
	        if err != nil {
	            panic(err.Error())
	        }
			count = count +1
	    ...
	    InItemRow = Inner_item{r.Id,r.Item_image, config.Img_to_render(`img_uploads/size_80/`+str_OrgID+`/`+r.Item_image+`.jpg`,r.Item_image,80,str_OrgID),r.Price ,r.Ucost,r.Sku ,r.Comm_per , r.Comm_amt , r.Remarks, r.Tax_per,r.Tax_amt,r.Ads_per,r.Ads_amt,r.Freight_per,r.Freight_amt}
	    }
