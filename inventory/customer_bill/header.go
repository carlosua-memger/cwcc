...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Location Name", "Status",  "Trans Date", "Customer Name", "Sold By","Remarks","Term"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/customer_bill?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_bill/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_bill/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_CusBilHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_CusBilHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		if(fromtype==`cp`){
			...
			cp_id :=r.URL.Query().Get("cp_id")
			fmt.Println(`SIS_CusSalHdr_Get 1,`+str_OrgID+`,`+cp_id )
			arr_data_co := datatables.Data_row(`SIS_CusSalHdr_Get 1,`+str_OrgID+`,`+cp_id )
			tconf["cp_id"] = 				arr_data_co[0]
			tconf["cp_status"] = 			arr_data_co[1] 
			tconf["cp_location_id"] = 		arr_data_co[2] 
			tconf["cp_location_name"] = 	arr_data_co[3]
			tconf["cp_trandate"] = 			arr_data_co[4] 
			tconf["cp_customer_name"] = 	arr_data_co[5] 
			tconf["cp_soldby_name"] = 		arr_data_co[6]
			tconf["cp_remarks"] = 			arr_data_co[7] 
			tconf["cp_customer_id"] = 	 	arr_data_co[8]
			tconf["cp_soldby_id"] = 		arr_data_co[9]
			tconf["cp_term"] = 				arr_data_co[10] 
			tconf["cp_courier_id"] = 		arr_data_co[11] 
			tconf["cp_courier_name"] = 		arr_data_co[12] 
		...
		arr_data_tag := datatables.DataList(`exec SIS_CusBilTags_List 1,`+str_OrgID+` , 0`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_bill/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_location,arr_data_supplier,arr_data_customer,arr_data_orgmem, arr_data_tag})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
...
func HAddTagHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
				fmt.Println(`SIS_CusBilTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
	        	arr_data_itemtag := datatables.Data_row(`SIS_CusBilTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_CusBilHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		arr_data_customer := datatables.DataList(`SIS_Customer_Get 2,`+str_OrgID  )
		arr_data_orgmem := datatables.DataList(`exec OrgMem_Get 1,`+str_OrgID+` , 0`  )
		arr_data_tag := datatables.DataList(`SIS_CusBilTags_List 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		

		tmpl := template.New("Hedit.html").Funcs(local_FuncMap) 
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_bill/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location,arr_data_supplier,arr_data_customer,arr_data_orgmem, arr_data_tag} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_bill/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_CusBilHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
