...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] = "Adjustment Entry Detail"
	...
	//tconf["Add_new_button_url"] ="/inventory/adjustment_entry/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Action_baseURL"] = "/inventory/adjustment_entry/details?rights=" + rights
	tconf["ActionCol_param"] = "ID"
	//tconf["hdr_id"] = ""
	tconf["Interpret_button"] = "false"
	tconf["Add_post_button"] = "false"
	tconf["Add_post_button_url"] = "/inventory/adjustment_entry/details/post?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]

	tconf["Add_cancel_button"] = "false"
	tconf["Add_cancel_button_url"] = "/inventory/adjustment_entry/details/cancel?rights=" + rights + "&hdr_id=" + tconf["hdr_id"]
	...
	if ln == "" {
		...
		arr_data := datatables.Data_row(`SIS_AdjEntHdr_Get 1 ,  ` + str_OrgID + ` ,'` + tconf["hdr_id"] + `'`)
		...
		tconf["suprec_hdr_remarks"] = arr_data[5]

		tconf["Add_new_button_url"] = "/inventory/adjustment_entry/details/DaddHandler?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&customer=" + tconf["suprec_hdr_customer"] + "&location=" + tconf["suprec_hdr_location"]
		// content access here
		// rights for tables
		...
		arr_data_tag := datatables.DataList(`SIS_AdjEntTags_List 1,` + str_OrgID + ` ,` + tconf["hdr_id"])

		var ajaxURLdata = "/inventory/adjustment_entry/details?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&status=" + tconf["suprec_hdr_status"] + "&location=" + tconf["suprec_hdr_location"]
		fmt.Println("/inventory/adjustment_entry/details?rights=" + rights + "&hdr_id=" + tconf["hdr_id"] + "&status=" + tconf["suprec_hdr_status"] + "&location=" + tconf["suprec_hdr_location"])
		type TblConf struct {
			Headercol []string
			AjaxUrl   string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/adjustment_entry/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol_edit_url"] = "/inventory/adjustment_entry/details/DEditHandler?rights=" + rights + "&location=" + location + "&h_id=" // config for edit click
		tconf["ActionCol_delete"] = "true"                                                                                                     // config for delete click
		tconf["ActionCol_delete_url"] = "/inventory/adjustment_entry/details/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_AdjEntDtl_List 1, ` + str_OrgID + `,` + hdr_id + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"] = `SIS_AdjEntDtl_List 0, ` + str_OrgID + `,` + hdr_id + `,  ` + str + `,  ` + ln + `, '` + sr + `'`
		tconf["org"] = str_OrgID
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/adjustment_entry/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		...
		*/
		...
		if isbarcode == `1` {
			...
			html_file = "inventory/adjustment_entry/Dadd_barcode.html"
		} else {
			html_filename = "Dadd.html"
			html_file = "inventory/adjustment_entry/Dadd.html"
		}
		...
		r.ParseForm()
		fmt.Println(`SIS_AdjEntDtl_Save 'Add', '` + username + `',0,` + r.Form["hdr_id"][0] + ` ,` + r.Form["item"][0] + `,` + r.Form["qty"][0] + `,` + r.Form["unit_cost"][0] + `,'` + r.Form["remarks"][0] + `'`)
		arr_raw_data := datatables.Data_row(`SIS_AdjEntDtl_Save 'Add', '` + username + `',0,` + r.Form["hdr_id"][0] + ` ,` + r.Form["item"][0] + `,` + r.Form["qty"][0] + `,` + r.Form["unit_cost"][0] + `,'` + r.Form["remarks"][0] + `'`)
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		arr_data := datatables.Data_row(`exec SIS_AdjEntDtl_Get 1, ` + str_OrgID + `, '` + tconf["h_id"] + `'`)
		...
		tconf["suprec_detail_ucost"] = arr_data[4]
		tconf["suprec_detail_remarks"] = arr_data[5]
		tconf["suprec_detail_hdr_id"] = arr_data[6]

		tmpl := template.New("Dedit.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/adjustment_entry/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w, &Data{rights, tconf})
		if err1 != nil {
		...
		r.ParseForm()
		fmt.Println(`SIS_AdjEntDtl_Save 'Edit', '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,` + r.Form["item"][0] + `,` + r.Form["qty"][0] + `,` + r.Form["unit_cost"][0] + `,'` + r.Form["remarks"][0] + `'`)
		arr_raw_data := datatables.Data_row(`SIS_AdjEntDtl_Save 'Edit', '` + username + `',` + r.Form["id"][0] + `,` + r.Form["hdr_id"][0] + ` ,` + r.Form["item"][0] + `,` + r.Form["qty"][0] + `,` + r.Form["unit_cost"][0] + `,'` + r.Form["remarks"][0] + `'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		tconf := make(map[string]string)
		tconf["h_id"] = r.URL.Query().Get("h_id")
		tconf["delete_url"] = "/inventory/adjustment_entry/details/DDeleteHandler"
		tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/adjustment_entry/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_AdjEntDtl_Save 'Delete','` + username + `',` + r.Form["h_id"][0])
...
func Ajax_getItem(w http.ResponseWriter, r *http.Request) {
	...
	if isbaseonassignid == `1` {
		...
		sqlstr = "exec SIS_Item_Get 17, " + str_OrgID + ",'" + getq + "'"
	} else {
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	username, _ := login.Get_account_info(r)
	r.ParseForm()
	fmt.Println(`SIS_AdjEnt_Post_Check ` + str_OrgID + `,` + hdr_id)
	arr_raw_data := datatables.DataList(`SIS_AdjEnt_Post_Check ` + str_OrgID + `,` + hdr_id)
	fmt.Println(arr_raw_data)
	if arr_raw_data == nil {
		fmt.Println(`GOOD TO POST`)
		arr_raw_data = datatables.DataList(`SIS_AdjEnt_Post '` + username + `',` + str_OrgID + `,` + hdr_id)
		fmt.Println(arr_raw_data)
	}
	js, err := json.Marshal(arr_raw_data)
	if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	username, _ := login.Get_account_info(r)
	r.ParseForm()
	fmt.Println(`SIS_AdjEnt_Cancel_Check ` + str_OrgID + `,` + hdr_id)
	//arr_data_tag := datatables.DataList(`SIS_SupRecTags_List 1,`+str_OrgID+` ,`+ tconf["hdr_id"] )

	arr_raw_data := datatables.DataList(`SIS_AdjEnt_Cancel_Check ` + str_OrgID + `,` + hdr_id)
	fmt.Println(arr_raw_data)
	if arr_raw_data == nil {
		fmt.Println(`GOOD TO POST`)
		arr_raw_data = datatables.DataList(`SIS_AdjEnt_Cancel '` + username + `',` + str_OrgID + `,` + hdr_id)
		fmt.Println(arr_raw_data)
	}
	js, err := json.Marshal(arr_raw_data)
	if err != nil {
