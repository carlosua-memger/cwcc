...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Location Name", "Status",  "Trans Date", "Customer Name", "Sold By","Application Balance","Application Status","Remarks","Term"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/customer_sales?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_application_is_modal"] ="false" // config for edit click 

	 	//tconf["ActionCol_edit_url"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_sales/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_sales/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_CusSalHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		
		tconf["sql_data"]  = `SIS_CusSalHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights , tconf,arr_data_location,arr_data_supplier,arr_data_customer,arr_data_orgmem,arr_data_courier, arr_data_tag})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
	...
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
	        	...
	        	arr_data_itemtag := datatables.Data_row(`SIS_CusSalTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_CusSalHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		tconf["suprec_cust_name"] = 		arr_data[5] 
		tconf["suprec_soldby_name"] = 		arr_data[6]
		tconf["suprec_remarks"] = 			arr_data[7] 
		tconf["suprec_custid"] = 	 		arr_data[8]
		tconf["suprec_soldby"] = 			arr_data[9] 
		tconf["suprec_term"] = 				arr_data[10] 
		tconf["suprec_courier"] = 			arr_data[11] 
		tconf["suprec_courier_name"] = 		arr_data[12] 
		...
		arr_data_location := datatables.DataList(`exec SIS_Location_Get 7,`+str_OrgID  )
		arr_data_supplier := datatables.DataList(`SIS_Supplier_Get 2,`+str_OrgID  )
		arr_data_customer := datatables.DataList(`sis_locationcustomer_get 4,`+tconf["suprec_location"] )
		arr_data_orgmem := datatables.DataList(`exec OrgMem_Get 1,`+str_OrgID+` , 0`  )
		arr_data_courier := datatables.DataList(`exec SIS_Courier_Get 2,`+str_OrgID  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location,arr_data_supplier,arr_data_customer,arr_data_orgmem, arr_data_courier, arr_data_tag} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_sales/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_CusSalHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
