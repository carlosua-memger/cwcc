...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Customer Sales Detail" 
	...
	//tconf["Add_new_button_url"] ="/inventory/customer_sales/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]
		// content access here
	tconf["Action_baseURL"] ="/inventory/customer_sales/details?rights="+rights
	tconf["ActionCol_param"] ="ID"
    //tconf["hdr_id"] = ""
    tconf["Archive"] ="false" // is archive
	tconf["Interpret_button"] ="false"
	tconf["Add_post_button"] ="false"
	tconf["Add_post_button_url"] ="/inventory/customer_sales/details/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	

	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/customer_sales/details/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Add_upload_button"] ="false"
	tconf["Add_upload_button_url"] ="/inventory/customer_sales/HaddHandler?rights="+rights+"&fromtype=cp&cp_id="+tconf["hdr_id"] 
	tconf["Add_duplicate_button"] ="false"
	tconf["Add_duplicate_button_url"] ="/inventory/customer_sales/details/duplicate_sales?rights="+rights+"&fromtype=cp&cp_id="+tconf["hdr_id"] 
	...
	tconf["Add_makepay_button"] ="false"
	tconf["Add_makepay_button_url"] ="/inventory/customer_payment/HaddHandler?rights="+rights+"&fromtype=co&co_id="+tconf["hdr_id"] 
	if last_length==""{
		tconf["pageLength"] ="10" 
		tconf["displayStart"] ="0"
	}
	var header = []string{"Action", "Item Name", "Qty", "Unit Price", "Total Price","Disc 1%","Disc 2%","Disc 3%", "Disc Amt", "Total Disc", "Disc Amt" ,"Tax %", "Tax Amt", "Total Tax","Comm %", "Comm Amt", "Total Comm","Ads %","Ads Amt","Ads Total","Freight %","Freight Amt","Freight Total","Net", "Remarks"}  //column header
	if ln=="" {
		
		tconf["ismoreinfo"] = "true" // to description info
		fmt.Println(`SIS_CusSalHdr_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'` )
		arr_data := datatables.Data_row(`SIS_CusSalHdr_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'`  )
		...
		tconf["suprec_hdr_corier_id"] = 	arr_data[11] 
		tconf["suprec_hdr_corier_name"] =   arr_data[12] 

		tconf["Add_new_button_url"] ="/inventory/customer_sales/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		tconf["Download_CusOrd_url"] ="/inventory/customer_sales/details/DownloadCusOrd?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		tconf["Email_url"] ="/inventory/customer_sales/details/email?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		// content access here
		// rights for tables 	
		...
		var ajaxURLdata = "/inventory/customer_sales/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]+"&location="+tconf["suprec_hdr_location"]+"&customer="+tconf["suprec_hdr_customer"]
		fmt.Println("/inventory/customer_sales/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]+"&location="+tconf["suprec_hdr_location"]+"&customer="+tconf["suprec_hdr_customer"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/customer_sales/details/DEditHandler?rights="+rights+"&customer="+customer+"&location="+location+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/customer_sales/details/DDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_CusSalDtl_List 1, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_CusSalDtl_List 0, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["org"] = str_OrgID
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/customer_sales/Dadd_barcode.html"
		}else{
			html_filename = "Dadd.html"
			html_file = "inventory/customer_sales/Dadd.html"
		}
		...
		fmt.Println(`SIS_CusSalDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_price"][0]+` ,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+` ,`+r.Form["discount_amt"][0]+` ,0 ,`+r.Form["taxper"][0]+` ,`+r.Form["taxamt"][0]+` ,0 ,`+r.Form["commper"][0]+` ,`+r.Form["commamt"][0]+` ,0,'`+r.Form["remarks"][0]+`','`+r.Form["adsper"][0]+`','`+r.Form["adsamt"][0]+`','`+r.Form["freightper"][0]+`','`+r.Form["freightamt"][0]+`'`)
		arr_raw_data := datatables.Data_row(`SIS_CusSalDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_price"][0]+` ,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+` ,`+r.Form["discount_amt"][0]+` ,0 ,`+r.Form["taxper"][0]+` ,`+r.Form["taxamt"][0]+` ,0 ,`+r.Form["commper"][0]+` ,`+r.Form["commamt"][0]+` ,0,'`+r.Form["remarks"][0]+`','`+r.Form["adsper"][0]+`','`+r.Form["adsamt"][0]+`','`+r.Form["freightper"][0]+`','`+r.Form["freightamt"][0]+`'` )
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`exec SIS_CusSalDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		arr_data := datatables.Data_row(`exec SIS_CusSalDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		tconf["suprec_detail_net"] 		= 	arr_data[16]
		tconf["suprec_detail_remarks"] = 	arr_data[17] 
		tconf["suprec_detail_hdr_id"] = 	arr_data[18] 
		tconf["suprec_detail_ads_per"] = 	arr_data[19] 
		tconf["suprec_detail_ads_amt"] = 	arr_data[20] 
		tconf["suprec_detail_freight_per"] =arr_data[21] 
		tconf["suprec_detail_freight_amt"] =arr_data[22] 
		tconf["suprec_detail_hdr_id"] =arr_data[23] 
		

		tmpl := template.New("Dedit.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
		...
		fmt.Println(`SIS_CusSalDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_price"][0]+` ,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+` ,`+r.Form["discount_amt"][0]+` ,0 ,`+r.Form["taxper"][0]+` ,`+r.Form["taxamt"][0]+` ,0 ,`+r.Form["commper"][0]+` ,`+r.Form["commamt"][0]+` ,0,'`+r.Form["remarks"][0]+`','`+r.Form["adsper"][0]+`','`+r.Form["adsamt"][0]+`','`+r.Form["freightper"][0]+`','`+r.Form["freightamt"][0]+`'`)
		arr_raw_data := datatables.Data_row(`SIS_CusSalDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+` ,`+r.Form["qty"][0]+` ,`+r.Form["unit_price"][0]+` ,`+r.Form["discount_per_1"][0]+`,`+r.Form["discount_per_2"][0]+`,`+r.Form["discount_per_3"][0]+` ,`+r.Form["discount_amt"][0]+` ,0 ,`+r.Form["taxper"][0]+` ,`+r.Form["taxamt"][0]+` ,0 ,`+r.Form["commper"][0]+` ,`+r.Form["commamt"][0]+` ,0,'`+r.Form["remarks"][0]+`',`+r.Form["adsper"][0]+`,`+r.Form["adsamt"][0]+`,`+r.Form["freightper"][0]+`,`+r.Form["freightamt"][0]+`` )
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
	    	http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/customer_sales/details/DDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_CusSalDtl_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
	...
	getq = getq + getid
		...
		str_OrgID :=strconv.Itoa(Org_id)
		 if (isbaseonassignid ==`1`){
		 	...
		 	sqlstr ="exec SIS_CusSal_Misc 4, "+str_OrgID+",0, "+hdr_id+",'"+getq+"'"
		 }else{
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
		...
		arr_raw_data := datatables.DataList(`SIS_CusSal_Post_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_CusSal_Post '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
		...
		arr_raw_data := datatables.DataList(`SIS_CusSal_Cancel_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_CusSal_Cancel '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DAdd_reference(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/Dadd_reference.html"); err != nil {
			fmt.Println(err)
		}
		err = tmpl.Execute(w,tconf)
		if err != nil {
...
func Show_prev_price(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/show_prev.html"); err != nil {
		fmt.Println(err)
	}
...
func Download_CusOrdHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	username, _ := login.Get_account_info(r)
	Org_id :=login.Get_session_org_id(r)
	str_OrgID :=strconv.Itoa(Org_id)
	...
	if r.Method =="GET" {	
		...
		arrdata := datatables.DataList(`SIS_CusSalHdr_Misc 1,`+hdr_id)
		fmt.Println(arrdata);
		html_filename = "download_CusOrd.html"
		html_file = "inventory/customer_sales/download_CusOrd.html"
...
func EmailHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		//arr_data := datatables.Data_row(`exec branch_get 1, 1,`+ tconf["h_id"]  )
		arr_data := datatables.Data_row(`exec SIS_CusSalHdr_Get 4, `+str_OrgID+`, `+hdr_id)
		...
		tconf["suprec_hdr_customername"] = 	arr_data[4] 
		tconf["suprec_hdr_membername"] = 	arr_data[5] 
		tconf["suprec_hdr_couriername"] = 	arr_data[6] 
		tconf["suprec_hdr_email"] = 		arr_data[7]
		tconf["suprec_hdr_org"] = 			arr_data[8] 
		tconf["suprec_hdr_locationemail"] = arr_data[9] 
		tconf["suprec_hdr_customer"] = arr_data[10] 
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/customer_sales/email.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
	...
	fmt.Println(`SIS_Customer_Get 4 ,  `+str_OrgID+` ,`+customer)
	arr_supplier := datatables.Data_row(`SIS_Customer_Get 4 ,  `+str_OrgID+` ,`+customer  )
	
	customer_name :=arr_supplier[0];
	org_name :=arr_supplier[1];


	
	fmt.Println(`SIS_CusSal_Custom 3,`+str_OrgID+` ,`+ hrd_id)
	SIS_SupOrdTags_List := datatables.DataList(`SIS_CusSal_Custom 3,`+str_OrgID+` ,`+ hrd_id )
	SIS_SupOrdHdr_Report := datatables.DataList(`SIS_CusSal_Custom 4,`+str_OrgID+` ,`+ hrd_id )
    ht_tbl:=`<table width="100%" cellpadding="15" style="table-layout: fixed;">
	            <tr>
	              	    <th align="right" bgcolor="#E5E7E9"><b>#</th>
                        <th bgcolor="#E5E7E9"><b>Item</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Qty</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Unit Price</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Total Price</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Disc 1 Percentage</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Disc 2 Percentage</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Disc 3 Percentage</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Disc Amount</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Total Discount</th>
                        <th align="right" bgcolor="#E5E7E9"><b>Discounted Amount</th>
	            </tr>        
			`;
    for _ , value := range SIS_SupOrdTags_List{
    	ht_tbl+=`<tr>`;
    		 ht_tbl+=`<td align="right">`+value[0]+`</td>`;
             ht_tbl+=`<td>`+value[1]+`</td>`;
             ht_tbl+=`<td align="right">`+value[2]+`</td>`;
             ht_tbl+=`<td align="right">`+value[3]+`</td>`;
             ht_tbl+=`<td align="right">`+value[4]+`</td>`;
             ht_tbl+=`<td align="right">`+value[5]+`</td>`;
             ht_tbl+=`<td align="right">`+value[6]+`</td>`;
             ht_tbl+=`<td align="right">`+value[7]+`</td>`;
             ht_tbl+=`<td align="right">`+value[8]+`</td>`;
	         ht_tbl+=`<td align="right">`+value[9]+`</td>`;
	         ht_tbl+=`<td align="right">`+value[10]+`</td>`;
    ...
    for _ , value2 := range SIS_SupOrdHdr_Report{
    	ht_tbl+=`<tr>`;
    		...
    		ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b>`+value2[0]+`</td>`;
            ht_tbl+=`<td bgcolor="#E5E7E9"><b></td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b>`+value2[1]+`</td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b></td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b>`+value2[2]+`</td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b></td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b></td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b></td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b></td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b>`+value2[3]+`</td>`;
            ht_tbl+=`<td align="right" bgcolor="#E5E7E9"><b>`+value2[4]+`</td>`;
	...
	e.Subject = org_name + ` Customer Sale (ID# ` +hrd_id+ `)`
	e.HTML = []byte(`Dear `+customer_name+`,<br><br>Sending you list of sales, as follows;<br><br>`+ht_tbl+`<br>Additional Message: `+r.Form["message"][0]+`<br>Thank you<br>`+org_name+`<br><br>This is a system generated email. Please send your reply to `+ r.Form["ccto"][0] + `.<br> Powered by <a href="www.memger.com"> Memger Tech Inc.</a> <i>Inventory like a <b>boss</b>... <a href="www.memger.com">try us for <b>free</b>.</a></i>`)
