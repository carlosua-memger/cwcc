...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Name", "Item Class Name", "Unit Price","SKU", "Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/item?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] ="/inventory/item/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/item/HDeleteHandler?h_id="
	 	tconf["ActionCol_detail"] ="false" // config for details click 
	 	tconf["ActionCol_add_child"] ="false" // config for addchild click 
	 	tconf["ActionCol_supplier"] ="true" // config for application click 
	 	tconf["ActionCol_duplicate"] ="false"
	 	tconf["ActionCol_duplicate_is_modal"] ="false"
	 	tconf["ActionCol_duplicate_url"] ="/inventory/item/HDuplicateHandler?rights="+rights+"&h_id="
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
