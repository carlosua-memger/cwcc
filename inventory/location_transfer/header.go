...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Location Transfer" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/location_transfer/HaddHandler?rights="+rights 
	...
	var header = []string{"Action", "ID", "Location Name", "Status",  "Trans Date", "Transfer To","Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/location_transfer?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/location_transfer/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/location_transfer/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_LocTraHdr_List 1, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_LocTraHdr_List 0, `+str_OrgID+`,0,  `+str+`,  `+ln+`, '`+sr+`',`+archive
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data_tag := datatables.DataList(`exec SIS_LocTraTags_List 1,`+str_OrgID+` , 0`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_transfer/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HAddTagHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
				fmt.Println(`SIS_LocTraTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
	        	arr_data_itemtag := datatables.Data_row(`SIS_LocTraTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_LocTraHdr_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		tconf["suprec_transferto"] = 		arr_data[5]  
		tconf["suprec_transfertoname"] = 	arr_data[6] 
		...
		arr_data_supplier := datatables.DataList(`SIS_Supplier_Get 2,`+str_OrgID  )
		arr_data_orgmem := datatables.DataList(`exec OrgMem_Get 1,`+str_OrgID+` , 0`  )
		arr_data_tag := datatables.DataList(`SIS_LocTraTags_List 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		

		tmpl := template.New("Hedit.html").Funcs(local_FuncMap) 
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_transfer/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_location,arr_data_supplier,arr_data_orgmem, arr_data_tag} )
		if err1 != nil {
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/location_transfer/HDeleteHandler"
		...
		arr_data := datatables.Data_row(`SIS_LocTraHdr_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		/*
		fmt.Println(arr_data) 
