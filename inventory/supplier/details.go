...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	//tconf["Add_new_button_url"] ="/inventory/supplier/items/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Action_baseURL"] ="/inventory/supplier/items?rights="+rights
	tconf["ActionCol_param"] ="ID"
    //tconf["hdr_id"] = ""
	tconf["Interpret_button"] ="false"
	tconf["Add_post_button"] ="false"
	tconf["Add_post_button_url"] ="/inventory/supplier/items/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/supplier/items/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	...
	if ln=="" {
		...
		arr_data := datatables.Data_row(`SIS_Supplier_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'`  )
		...
		tconf["customer_hdr_remarks"] = 			arr_data[9]
		tconf["Panel_name"] = tconf["customer_hdr_name"] + " Items"
		...
		tconf["Add_new_button_url"] ="/inventory/supplier/items/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&item="+tconf["hdr_id"]
		...
		var ajaxURLdata = "/inventory/supplier/items?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+"&item="+tconf["hdr_id"]
		fmt.Println("/inventory/supplier/items?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+"&item="+tconf["hdr_id"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/supplier/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/supplier/items/DEditHandler?rights="+rights+"&hdr_id="+hdr_id+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/supplier/items/DDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_SupplierItem_List 1,`+hdr_id+`, 0, `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_SupplierItem_List 0,`+hdr_id+`, 0, `+str+`,  `+ln+`, '`+sr+`'`
...
func DaddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data_customer := datatables.DataList(`SIS_Item_Get 13,`+str_OrgID+`,'`+tconf["hdr_id"]+`'`   )
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/supplier/Dadd.html"
		}
		...
		fmt.Println(`SIS_SupplierItem_Save 'Add', '`+username+`',0, '`+str_OrgID+`' ,`+hrd_id+`,`+checkbox_id+`,'`+code+`',1,'`+qty+`'` )
		arr_raw_data := datatables.Data_row(`SIS_SupplierItem_Save 'Add', '`+username+`',0, '`+str_OrgID+`',`+hrd_id+`,`+checkbox_id+`,'`+code+`',1,'`+qty+`'`  )
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/supplier/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf,arr_data_customer} )
		if err1 != nil {
		...
		r.ParseForm()
		fmt.Println(`SIS_SupplierItem_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`, '`+str_OrgID+`' ,`+r.Form["hdr_id"][0]+`,`+r.Form["location"][0]+`,'`+r.Form["code"][0]+`',`+r.Form["activated"][0]+`,'`+r.Form["remarks"][0]+`'`  )
		arr_raw_data := datatables.Data_row(`SIS_SupplierItem_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`, '`+str_OrgID+`',`+r.Form["hdr_id"][0]+`,`+r.Form["location"][0]+` ,'`+r.Form["code"][0]+`',`+r.Form["activated"][0]+`,'`+r.Form["remarks"][0]+`'` )
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
	    	http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/supplier/items/DDeleteHandler"
