...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Supplier Master List" 
	...
	if ln=="" {
		...
		var ajaxURLdata = "/inventory/supplier?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	//tconf["ActionCol_item_url"] ="/inventory/supplier/items?rights="+rights+"&h_id="
	 	tconf["ActionCol_item"] ="true" // config for application click 
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
