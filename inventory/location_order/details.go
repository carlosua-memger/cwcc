...
func DListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Location Order Detail" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="true"
	tconf["Add_new_button_details"] ="true" 
	tconf["upload_button"] ="false"   
	//tconf["Add_new_button_url"] ="/inventory/location_order/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["Action_baseURL"] ="/inventory/location_order/details?rights="+rights
	tconf["ActionCol_param"] ="ID"
    //tconf["hdr_id"] = ""
	tconf["Interpret_button"] ="false"
	tconf["Add_post_button"] ="false"
	tconf["Add_post_button_url"] ="/inventory/location_order/details/post?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	tconf["ActionCol_void"] ="true" // config for void click 
	tconf["ActionCol_void_url"] ="/inventory/location_order/details/DVoidHandler?h_id="
	tconf["Add_cancel_button"] ="false"
	tconf["Add_cancel_button_url"] ="/inventory/location_order/details/cancel?rights="+rights+"&hdr_id="+tconf["hdr_id"]
	...
	if ln=="" {
		...
		arr_data := datatables.Data_row(`SIS_LocOrdHdr_Get 1 ,  `+str_OrgID+` ,'`+tconf["hdr_id"]+`'`  )
		...
		tconf["Add_new_button_url"] ="/inventory/location_order/details/DaddHandler?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&customer="+tconf["suprec_hdr_customer"]+"&location="+tconf["suprec_hdr_location"]
		...
		var ajaxURLdata = "/inventory/location_order/details?rights="+rights+"&hdr_id="+tconf["hdr_id"]+"&status="+ tconf["suprec_hdr_status"]
		fmt.Println("/inventory/location_order/details?rights="+rights+"&hdr_id="+tconf["hdr_id"])
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/location_order/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] =  "/inventory/location_order/details/DEditHandler?rights="+rights+"&location="+location+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/location_order/details/DDeleteHandler?h_id="
...
/*		fmt.Println("exec LBR_LogHdr_List 1, 2, 1, 1, 1, '"+sortColName+"', '', '"+sr+"'	" )
		...
		tconf["sql_total"] = `SIS_LocOrdDtl_List 1, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
		//tconf["sql_data"]  = `SIS_Item_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_LocOrdDtl_List 0, `+str_OrgID+`,`+hdr_id+`,  `+str+`,  `+ln+`, '`+sr+`'`
...
func DAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_order/Dadd.html"); err != nil {
			fmt.Println(err)
		}
		...
		if (isbarcode == `1`){
			...
			html_file = "inventory/location_order/Dadd_barcode.html"
		}else{
			html_filename = "Dadd.html"
			html_file = "inventory/location_order/Dadd.html"
		}
		...
		r.ParseForm()
		fmt.Println(`SIS_LocOrdDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,0,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_LocOrdDtl_Save 'Add', '`+username+`',0,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,0,'`+r.Form["remarks"][0]+`'` )
...
func DEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_LocOrdDtl_Get 1, `+str_OrgID+`, '`+tconf["h_id"]+`'`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_order/Dedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{rights,tconf} )
		if err1 != nil {
		...
		r.ParseForm()
		fmt.Println(`SIS_LocOrdDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,0,'`+r.Form["remarks"][0]+`'` )
		arr_raw_data := datatables.Data_row(`SIS_LocOrdDtl_Save 'Edit', '`+username+`',`+r.Form["id"][0]+`,`+r.Form["hdr_id"][0]+` ,`+r.Form["item"][0]+`,`+r.Form["qty"][0]+`,0,'`+r.Form["remarks"][0]+`'`)
		fmt.Println(arr_raw_data)
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
	    	http.Error(w, err.Error(), http.StatusInternalServerError)
...
func DDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/location_order/details/DDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_order/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_LocOrdDtl_Save 'Delete','`+username+`',` +r.Form["h_id"][0]  )
	...
	getq = getq + getid
		...
		str_OrgID :=strconv.Itoa(Org_id)
		 if (isbaseonassignid ==`1`){
		 	...
		 	sqlstr ="exec SIS_LocOrd_Misc 1, "+str_OrgID+",0, "+hdr_id+",'"+getq+"'"
		 }else{
...
func DPostHandler(w http.ResponseWriter, r *http.Request) {
		...
		username, _ := login.Get_account_info(r)
		r.ParseForm()
		fmt.Println(`SIS_LocOrd_Post_Check `+str_OrgID+`,`+hdr_id )
		arr_raw_data := datatables.DataList(`SIS_LocOrd_Post_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_LocOrd_Post '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DCancelHandler(w http.ResponseWriter, r *http.Request) {
		...
		username, _ := login.Get_account_info(r)
		r.ParseForm()
		fmt.Println(`SIS_LocOrd_Cancel_Check `+str_OrgID+`,`+hdr_id )
		//arr_data_tag := datatables.DataList(`SIS_SupRecTags_List 1,`+str_OrgID+` ,`+ tconf["hdr_id"] )

		arr_raw_data := datatables.DataList(`SIS_LocOrd_Cancel_Check `+str_OrgID+`,`+hdr_id)
		fmt.Println(arr_raw_data)
		if(arr_raw_data==nil){
			fmt.Println(`GOOD TO POST`)
			arr_raw_data = datatables.DataList(`SIS_LocOrd_Cancel '`+username+`',`+str_OrgID+`,`+hdr_id)
			fmt.Println(arr_raw_data)
		}
		js, err := json.Marshal(arr_raw_data)
		if err != nil {
...
func DAdd_reference(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_order/Dadd_reference.html"); err != nil {
			fmt.Println(err)
		}
		err = tmpl.Execute(w,tconf)
		if err != nil {
	...
	getq = getq + getid
		...
		str_OrgID :=strconv.Itoa(Org_id)
		 if (isbaseonassignid ==`1`){
		 	...
		 	fmt.Println(sqlstr)
		 	sqlstr = "sis_tranref_list 2, 3, 6,"+h_id
		 }else{
		 	//sqlstr = "exec LBR_Member_Get "+strtype+",  '"+getq+"'"
...
func DVoidHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/location_order/details/DVoidHandler"
	   	tmpl := template.New("modal_void_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location_order/modal_void_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		err = tmpl.Execute(w,tconf)
		if err != nil {
		...
		r.ParseForm()
		username, _ := login.Get_account_info(r)
		arr_data := datatables.Data_row(`SIS_LocOrd_Void '`+username+`',` +r.Form["h_id"][0]  )
		js, err := json.Marshal(arr_data)
		if err != nil {
	    	http.Error(w, err.Error(), http.StatusInternalServerError)
	    	return
