...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	var header = []string{"Action", "ID", "Name","Contact Person", "Address", "Contact Number","Email", "Remarks"}  //column header
	if ln=="" {
		// content access here
		...
		var ajaxURLdata = "/inventory/courier?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		if tmpl, err = tmpl.ParseFiles("inventory/courier/table.html"); err != nil {
			fmt.Println(err)
		}
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	tconf["ActionCol_edit_url"] ="/inventory/courier/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/courier/HDeleteHandler?h_id="
		...
		tconf["sql_total"] = `SIS_Courier_List 1, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_Courier_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
...
var local_FuncMap = template.FuncMap{
    	"Sql_list": func(s_qry string , org_id string, tag_id string) [][]string{
			fmt.Println(`SIS_CouTags_Get 1, `+org_id+`, 0 ,`+tag_id)
			retdata := datatables.DataList(`SIS_CouTags_Get 1, `+org_id+`, 0 ,`+tag_id)
			return retdata
		},
	}
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data_itemtag := datatables.DataList(`SIS_CouTags_List 1,`+str_OrgID+` , 0`  )
		type Data struct {
			Rights	string	
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/courier/Hadd.html"); err != nil {
			fmt.Println(err)
		}
...
func HAddTagHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
	        	arr_data_itemtag := datatables.Data_row(`SIS_CouTags_Save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', '`+remarks+`'` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_Courier_Get 1, '`+str_OrgID+`', '`+tconf["h_id"]+`'`  )
		
		fmt.Println(arr_data) 
		tconf["suprec_id"] = 		arr_data[0]
		tconf["item_org"] = 		arr_data[1] 
		tconf["item_name"] = 		arr_data[2]
		tconf["item_contactperson"] = arr_data[3]
		tconf["item_tin"] = 		arr_data[4]  
		tconf["item_address"] = 	arr_data[5]
		tconf["item_area"] = 		arr_data[6]
		tconf["item_email"] = 		arr_data[7]
		tconf["item_contactno"] = 	arr_data[8]     
		tconf["item_remarks"] = 	arr_data[9] 
		
		//ls_itemclass:=datatables.DataList(`exec SIS_Supplier_Get 2, `+str_OrgID+``)
		arr_data_itemtag := datatables.DataList(`SIS_CouTags_List 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		//fmt.Println(ls_itemclass)

		tmpl := template.New("Hedit.html").Funcs(local_FuncMap)
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/courier/Hedit.html"); err != nil {
			fmt.Println(err)
		}
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	...
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/courier/HDeleteHandler"
	   	tmpl := template.New("modal_delete_loghdr.html")
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/courier/modal_delete_loghdr.html"); err != nil {
			fmt.Println(err)
		}
		...
		fmt.Println(`SIS_Courier_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +` , `+str_OrgID+`` )
		
		arr_data := datatables.Data_row(`SIS_Courier_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +``)
		/*
		fmt.Println(arr_data) 
