...
func HListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tconf["Panel_name"] ="Location Master List" 
	tconf["Add_new_button"] ="false" 
	tconf["Rights"] = rights
	tconf["Add_back_button"] ="false"
	tconf["Add_new_button_details"] ="false" 
	tconf["upload_button"] ="false"   
	tconf["Add_new_button_url"] ="/inventory/location/HaddHandler?rights="+rights 
	//tconf["Action_baseURL"] ="/inventory/location/customers?rights="+rights
	tconf["Action_baseURL"] ="/inventory/location/customer?rights="+rights //application link 	
	...
	var header = []string{"Action", "ID", "Name","Address","Postal Code", "Email", "Remarks"}  //column header
	if ln=="" {
		...
		if( strings.Contains(dec_rights, "HAdd") ){
			...
			tconf["ActionCol_add_child_inherit"] ="true"
		}
		...
		var ajaxURLdata = "/inventory/location?rights="+rights
		type TblConf struct{
			Headercol []string
			AjaxUrl string
		...
		tconf["ActionCol"] ="true" // config for action column
	 	...
	 	//tconf["ActionCol_edit_u5rl"] ="/timekeeping/overtime_logs/OvertimeLogsHeaderEdit?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_edit_url"] ="/inventory/location/HEditHandler?rights="+rights+"&h_id=" // config for edit click 
	 	tconf["ActionCol_delete"] ="false" // config for delete click 
	 	tconf["ActionCol_delete_url"] ="/inventory/location/HDeleteHandler?h_id="
	 	tconf["ActionCol_detail"] ="false" // config for details click 
	 	tconf["ActionCol_add_child_inherit"] ="false" // config for addchild click 
	 	tconf["ActionCol_add_child_inherit_url"] ="/inventory/location/HAddChildHandler?rights="+rights+"&h_id="
	 	//tconf["ActionCol_customer_url"] ="/inventory/location_items/customers?rights="+rights+"&h_id="
	 	tconf["ActionCol_customer"] ="true" // config for application click 
	 	tconf["style_size"] ="12"
	 	tconf["style_font"] =""
		...
		tconf["sql_total"] = `SIS_Location_List 1, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
		tconf["sql_data"]  = `SIS_Location_List 0, `+str_OrgID+`,  `+str+`,  `+ln+`, '`+sr+`'`
...
var local_FuncMap = template.FuncMap{
    	"Sql_list": func(s_qry string , org_id string, tag_id string) [][]string{
			fmt.Println(`sis_loctags_get 1, `+org_id+`, 0 ,`+tag_id)
			retdata := datatables.DataList(`sis_loctags_get 1, `+org_id+`, 0 ,`+tag_id)
			return retdata
		},
	}
...
func HAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		fmt.Println(`parent ID here`)
		fmt.Println(tconf["parentID"])
		tconf["username"] =username
		tconf["Org_id"] = str_OrgID
		...
		arr_data_itemtag := datatables.DataList(`sis_loctags_list 1,`+str_OrgID+` , 0`  )
		...
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location/Hadd.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute(w,&Data{rights,tconf,arr_data_itemtag,arr_data_location,arr_data_customer})
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
...
func HaddTagHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method =="POST" {
		...
		for key ,_ := range r.Form["tag"] {
			...
			if(value_input!=""){
	        	arr_data_itemtag := datatables.Data_row(`SIS_loctags_save 'Update', '`+username+`',`+str_OrgID+`,`+item_id+`, `+tag+`, '`+value_input+`', ''` )
...
func HEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {	
		...
		arr_data := datatables.Data_row(`exec SIS_Location_Get 1, '`+str_OrgID+`', '`+tconf["h_id"]+`'`  )
		
		fmt.Println(arr_data) 
		tconf["item_id"] = 			arr_data[0]
		tconf["item_org"] = 		arr_data[1] 
		tconf["item_name"] = 		arr_data[2] 
		tconf["item_address"] = 	arr_data[3]
		tconf["item_postcode"] = 	arr_data[4]
		tconf["item_email"] = 		arr_data[5]
		tconf["item_remarks"] = 	arr_data[6] 
		tconf["item_parent"] = 	arr_data[7] 
		tconf["item_customer"] = 	arr_data[8] 


		
		//ls_itemclass:=datatables.DataList(`exec SIS_Supplier_Get 2, `+str_OrgID+``)
		arr_data_location := datatables.DataList(`exec SIS_Location_Get 6,`+str_OrgID   )
		arr_data_itemtag := datatables.DataList(`sis_loctags_list 1,`+str_OrgID+` ,`+ tconf["h_id"] )
		arr_data_customer := datatables.DataList(`SIS_Customer_Get 2,`+str_OrgID  )
		//fmt.Println(ls_itemclass)

		tmpl := template.New("Hedit.html").Funcs(local_FuncMap)
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location/Hedit.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{tconf,arr_data_itemtag,arr_data_location,arr_data_customer} )
		if err1 != nil {
			http.Error(w, err1.Error(), http.StatusInternalServerError)
		}
	}else{
...
func HDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method =="GET" {
	   	...
	   	tconf := make(map[string]string)
	   	tconf["h_id"] = r.URL.Query().Get("h_id")
	   	tconf["delete_url"] = "/inventory/location/HDeleteHandler"
		...
		fmt.Println(`SIS_Location_Save 'Delete','`+username+`',` +r.Form["h_id"][0] )
		
		arr_data := datatables.Data_row(`SIS_Location_Save 'Delete','`+username+`' ,  `+r.Form["h_id"][0] +`` )
		/*
		fmt.Println(arr_data) 
		tconf["item_id"] = 			arr_data[0]
		tconf["item_org"] = 		arr_data[1] 
...
func HAddChildHandler(w http.ResponseWriter, r *http.Request) {
	login.Session_renew(w,r)
	rights :=r.URL.Query().Get("rights")
	Org_id :=login.Get_session_org_id(r)
	str_OrgID :=strconv.Itoa(Org_id)
	...
	if r.Method =="GET" {	
		...
		//arr_data := datatables.Data_row(`exec branch_get 1, 1,`+ tconf["h_id"]  )
		arr_data := datatables.Data_row(`exec SIS_Location_Get 10, '`+str_OrgID+`', '`+tconf["h_id"]+`'`  )
		...
		tconf["item_parent"] = 	arr_data[7] 
		tconf["item_customer"] = 	arr_data[8] 
		tconf["item_location"] = 	arr_data[9] 
		...
		tmpl := template.New("HAddChild.html").Funcs(local_FuncMap)
		var err error
		if tmpl, err = tmpl.ParseFiles("inventory/location/HAddChild.html"); err != nil {
			fmt.Println(err)
		}
		err1 := tmpl.Execute( w,&Data{tconf,arr_data_itemtag,arr_data_location,arr_data_customer} )
		if err1 != nil {
